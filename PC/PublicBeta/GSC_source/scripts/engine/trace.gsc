// scripts\engine\trace.gsc  (62CEB051D758BFE.gscc) (size: 17216 Bytes / 0x4340 / GSC)
// magic .... 0xa0d4353478a vm: Call of Duty: Modern Warfare III (PC)
// crc: 0xc016f331 (3222729521)
// size ...... 17216 (0x4340)
// includes .. 3   (offset: 0x4ca)
// strings ... 48  (offset: 0x3d48)
// dev strs .. 33  (offset: 0x40a0)
// exports ... 53  (offset: 0x298c)
// cseg ...... 0x4e2 + 0x24aa (0x298c)
// imports ... 79  (offset: 0x31d4)
// animtree1 . 0   (offset: 0x4340)
// animtree2 . 0   (offset: 0x4340)
...
function ray_trace(start, end, ignore, contentoverride, getsurfacetype, ignoreclutter, var_b00c60a54d663ca2) {
    ...
    trace = physics_raycast(start, end, content, ignore, 0, "physicsquery_closest", var_f6dbad2cd5a99bb1, var_b00c60a54d663ca2);
    if (trace.size) {
        trace = trace[0];
    } else {
...
// Size: 0x150
function create_contents(character, world, glass, shotclip, item, vehicle, playerclip, ainosight, itemclip, vehicleclip, waterclip) {
    content = 0;
    if (isdefined(character) && character) {
        content += create_character_contents();
    }
    if (isdefined(world) && world) {
        content += create_world_contents();
    }
    if (isdefined(glass) && glass) {
        content += create_glass_contents();
    }
    if (isdefined(shotclip) && shotclip) {
        content += create_shotclip_contents();
    }
    if (isdefined(item) && item) {
        content += create_item_contents();
    }
    if (isdefined(vehicle) && vehicle) {
        content += create_vehicle_contents();
    }
    if (isdefined(playerclip) && playerclip) {
        content += create_playerclip_contents();
    }
    if (isdefined(ainosight) && ainosight) {
        content += create_ainosight_contents();
    }
    if (isdefined(itemclip) && itemclip) {
        content += create_itemclip_contents();
    }
    if (isdefined(vehicleclip) && vehicleclip) {
        content += function_f3dae12daeb1e8bb();
    }
    if (isdefined(waterclip) && waterclip) {
        content += function_b88b8bd51ea7fe24();
    }
    return content;
}
