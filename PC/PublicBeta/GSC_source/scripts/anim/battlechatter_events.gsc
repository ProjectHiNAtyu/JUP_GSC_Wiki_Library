// scripts\anim\battlechatter_events.gsc  (50EEB9595C6D6E1B.gscc) (size: 325947 Bytes / 0x4f93b / GSC)
// magic .... 0xa0d4353478a vm: Call of Duty: Modern Warfare III (PC)
// crc: 0x44793d62 (1148796258)
// size ...... 325947 (0x4f93b)
// includes .. 10  (offset: 0x11b6c)
// strings ... 3413 (offset: 0x3cfcb)
// dev strs .. 15  (offset: 0x4f87f)
// exports ... 123 (offset: 0x39cfb)
// cseg ...... 0x11bbc + 0x2813f (0x39cfb)
// imports ... 213 (offset: 0x3b033)
// animtree1 . 0   (offset: 0x4f93b)
// animtree2 . 0   (offset: 0x4f93b)
...
// Namespace battlechatter_events / scripts\anim\battlechatter_events
// Params 1, eflags: 0x2 linked
// Checksum 0x0, Offset: 0x12526
// Size: 0xd9, Type: bool
function function_8d862c3d37cb8ebd(enemy) {
    if (!isdefined(enemy) || !isdefined(enemy.origin) || !isdefined(enemy.team) || !isai(enemy)) {
        return false;
    }
...
function function_f06007e1f432ce27() {
    ...
    if (guys.size == 2) {
        ...
        foreach (alias in aliases) {
            convo[convo.size] = randomfloatrange(1, 2);
    ...
    if (var_542b15e7d1da2adb == 1) {
        ...
        sequence = [];
        foreach (alias in aliases) {
            sequence[sequence.size] = randomfloatrange(1, 2);
...
// Checksum 0x0, Offset: 0x15edc
// Size: 0x87
function private function_bfcde1fbf9a6fcc5(player) {
    self endon("bc_hunt_cleared");
    player endon("bc_hunt_cleared");
    thread function_2e55246093b4bb17(player);
    player waittill("bc_combat_hunt_cleared");
    wait 2;
    self.var_f176a4709196bbbc = 1;
    leader = function_592f13f1a0e078ff(self);
    other = function_4375748e6ffa4aae(player, leader);
    while (true) {
        if (!isdefined(other) || !isalive(other)) {
            other = getclosestfriendlyspeaker();
        }
        function_607987e6e8f08716(player, leader, other);
    }
}

// Namespace battlechatter_events / scripts\anim\battlechatter_events
// Params 1, eflags: 0x6 linked
// Checksum 0x0, Offset: 0x15f6b
// Size: 0x52
function private function_2e55246093b4bb17(player) {
    msg = waittill_any_ents_return(self, "death", self, "bcs_stateChange", player, "death_or_disconnect");
    if (isdefined(self)) {
        self notify("bc_hunt_cleared");
    }
    if (isdefined(player)) {
        player notify("bc_hunt_cleared");
    }
    self.var_f176a4709196bbbc = undefined;
}
...
// Size: 0x370
function private function_4375748e6ffa4aae(player, leader) {
...
// Checksum 0x0, Offset: 0x1633d
// Size: 0x317
function private function_607987e6e8f08716(player, leader, other) {
    if (isdefined(other)) {
        ...
        if (!other_isvalid || var_b70b49dc45959168) {
            ...
            var_b70b49dc45959168 = other isnear(soldier1, level.var_8e8e487a64d9aa1b);
            if (other_isvalid && !var_b70b49dc45959168) {
    ...
    sequence_solo = [leader, "report_search", 0.6, self, "report_searching", 0.6, leader, "confirming", 0.6, "order_continue_search", 0.6, "sign_off", 0.6];
...
// Namespace battlechatter_events / scripts\anim\battlechatter_events
// Params 0, eflags: 0x2 linked
// Checksum 0x0, Offset: 0x16f62
// Size: 0xd1
function function_f4671f61a3031990() {
    self endon("bc_combat_hunt_ai_na");
    if (is_dead_or_dying(self) || istrue(self._blackboard.invehicle) || isdefined(self.var_9f44126029ba3947)) {
        return;
    }
    foreach (player in getplayersinrange(self.origin, level.var_3c8c2ca8a1d0e8a7)) {
        if (!isalive(player) || istrue(player.inlaststand) || isdefined(player.var_9f44126029ba3947)) {
            continue;
        }
        childthread function_cdf55cf331718d81(player);
        childthread function_bfcde1fbf9a6fcc5(player);
    }
}

// Namespace battlechatter_events / scripts\anim\battlechatter_events
// Params 0
// Checksum 0x0, Offset: 0x1703b
// Size: 0xa8
...
// Namespace battlechatter_events / scripts\anim\battlechatter_events
// Params 0, eflags: 0x2 linked
// Checksum 0x0, Offset: 0x170eb
// Size: 0x131
function function_da9e3311b63335e1() {
    self endon("death");
    self endon("bcs_stateChange");
    ...
    if (var_2bd00dd4702caf70 && isdefined(other.stealth) && other.var_fe5ebefa740c7106 == 3 && gettime() - other lastknowntime(self.enemy) > getdvarfloat(@"hash_57a6e76d28ffdee2")) {
        chatter = function_93b4f17e4a3474b("lost_target_elapsed_2", [self, "neg_target", 0.7]);
        waitchatter(chatter);
    }
}
...
function function_1ae088b4c06d0525(previous_state, ai_event, var_de837dfd8bcdc3af) {
    ...
    if ((ai_event == "gunshot" || ai_event == "gunshot_impact" || ai_event == "bulletwhizby") && !var_63f509639a8c705) {
...
// Namespace battlechatter_events / scripts\anim\battlechatter_events
// Params 0, eflags: 0x2 linked
// Checksum 0x0, Offset: 0x18ee1
// Size: 0x4e
function function_7825e0643a6c5600() {
    radio = function_592f13f1a0e078ff(self);
    self_name = function_2008b43adf7e18ef();
    function_93b4f17e4a3474b("reacquire_push_towards", [radio, self_name, 0.2, "order_check_last"]);
}
...
function event_init() {
    ...
    level.battlechatter.eventstrings["RU"]["idle_solo"][0] = ["s_wala_isc1line1", "s_wala_isc1line2", "s_wala_isc1line3", "s_wala_isc1line4", "s_wala_isc1line5", "s_wala_isc1line6", "s_wala_isc1line7", "s_wala_isc1line8"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][1] = ["s_wala_isc2line1", "s_wala_isc2line2", "s_wala_isc2line3", "s_wala_isc2line4", "s_wala_isc2line5", "s_wala_isc2line6", "s_wala_isc2line7", "s_wala_isc2line8", "s_wala_isc2line9", "s_wala_isc2line1_01"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][2] = ["s_wala_isc3line1", "s_wala_isc3line2", "s_wala_isc3line3", "s_wala_isc3line4", "s_wala_isc3line5", "s_wala_isc3line6", "s_wala_isc3line7", "s_wala_isc3line8", "s_wala_isc3line9", "s_wala_isc3line1_01", "s_wala_isc3line11", "s_wala_isc3line12", "s_wala_isc3line13", "s_wala_isc3line14"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][3] = ["s_wala_isc4line1", "s_wala_isc4line2", "s_wala_isc4line3", "s_wala_isc4line4", "s_wala_isc4line5", "s_wala_isc4line6", "s_wala_isc4line7", "s_wala_isc4line8", "s_wala_isc4line9", "s_wala_isc4line1_01", "s_wala_isc4line11", "s_wala_isc4line12", "s_wala_isc4line13"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][4] = ["s_wala_isc5line1", "s_wala_isc5line2", "s_wala_isc5line3", "s_wala_isc5line4", "s_wala_isc5line5", "s_wala_isc5line6", "s_wala_isc5line7", "s_wala_isc5line8", "s_wala_isc5line9", "s_wala_isc5line1_01", "s_wala_isc5line11", "s_wala_isc5line12", "s_wala_isc5line13", "s_wala_isc5line14"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][5] = ["s_wala_isc6line1", "s_wala_isc6line2", "s_wala_isc6line3", "s_wala_isc6line4", "s_wala_isc6line5", "s_wala_isc6line6", "s_wala_isc6line7", "s_wala_isc6line8", "s_wala_isc6line9", "s_wala_isc6line1_01", "s_wala_isc6line11", "s_wala_isc6line12", "s_wala_isc6line13", "s_wala_isc6line14", "s_wala_isc6line15", "s_wala_isc6line16"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][6] = ["s_wala_isc7line1", "s_wala_isc7line2", "s_wala_isc7line3", "s_wala_isc7line4", "s_wala_isc7line5", "s_wala_isc7line6", "s_wala_isc7line7", "s_wala_isc7line8", "s_wala_isc7line9", "s_wala_isc7line1_01", "s_wala_isc7line11", "s_wala_isc7line12", "s_wala_isc7line13", "s_wala_isc7line14", "s_wala_isc7line15", "s_wala_isc7line16", "s_wala_isc7line17", "s_wala_isc7line18", "s_wala_isc7line19", "s_wala_isc7line2_01", "s_wala_isc7line20", "s_wala_isc7line21", "s_wala_isc7line22", "s_wala_isc7line23", "s_wala_isc7line24", "s_wala_isc7line25", "s_wala_isc7line26", "s_wala_isc7line27", "s_wala_isc7line28"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][7] = ["s_wala_isc8line1", "s_wala_isc8line2", "s_wala_isc8line3", "s_wala_isc8line4", "s_wala_isc8line5", "s_wala_isc8line6", "s_wala_isc8line7", "s_wala_isc8line8", "s_wala_isc8line9", "s_wala_isc8line1_01", "s_wala_isc8line11", "s_wala_isc8line12", "s_wala_isc8line13", "s_wala_isc8line14", "s_wala_isc8line15", "s_wala_isc8line16", "s_wala_isc8line17", "s_wala_isc8line18", "s_wala_isc8line19", "s_wala_isc8line2_01", "s_wala_isc8line20", "s_wala_isc8line21", "s_wala_isc8line22", "s_wala_isc8line23", "s_wala_isc8line24"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][8] = ["s_wala_isc9line1", "s_wala_isc9line2", "s_wala_isc9line3", "s_wala_isc9line4", "s_wala_isc9line5", "s_wala_isc9line6", "s_wala_isc9line7", "s_wala_isc9line8", "s_wala_isc9line9", "s_wala_isc9line1_01", "s_wala_isc9line11", "s_wala_isc9line12", "s_wala_isc9line13", "s_wala_isc9line14", "s_wala_isc9line15"];
    level.battlechatter.eventstrings["RU"]["idle_solo"][9] = ["s_wala_isc10line1", "s_wala_isc10line2", "s_wala_isc10line3", "s_wala_isc10line4", "s_wala_isc10line5", "s_wala_isc10line6", "s_wala_isc10line7", "s_wala_isc10line8", "s_wala_isc10line9", "s_wala_isc10line1_01", "s_wala_isc10line11", "s_wala_isc10line12", "s_wala_isc10line13", "s_wala_isc10line14", "s_wala_isc10line15", "s_wala_isc10line16", "s_wala_isc10line17", "s_wala_isc10line18", "s_wala_isc10line19", "s_wala_isc10line2_01", "s_wala_isc10line20"];
    level.battlechatter.eventstrings["RU"]["idle_conv"] = [];
    level.battlechatter.eventstrings["RU"]["idle_conv"][0] = ["s_wala_wallaconversation01l", "s_wala_wallaconversation01l_01", "s_wala_wallaconversation01l_02", "s_wala_wallaconversation01l_03", "s_wala_wallaconversation01l_04", "s_wala_wallaconversation01l_05", "s_wala_wallaconversation01l_06", "s_wala_wallaconversation01l_07", "s_wala_wallaconversation01l_08", "s_wala_wallaconversation01l_09", "s_wala_wallaconversation01l_10", "s_wala_wallaconversation01l_11", "s_wala_wallaconversation01l_12"];
    level.battlechatter.eventstrings["RU"]["idle_conv"][1] = ["s_wala_wallaconversation02l", "s_wala_wallaconversation02l_01", "s_wala_wallaconversation02l_02", "s_wala_wallaconversation02l_03", "s_wala_wallaconversation02l_04", "s_wala_wallaconversation02l_05", "s_wala_wallaconversation02l_06", "s_wala_wallaconversation02l_07", "s_wala_wallaconversation02l_08"];
    level.battlechatter.eventstrings["RU"]["idle_conv"][2] = ["s_wala_wallaconversation03l", "s_wala_wallaconversation03l_01", "s_wala_wallaconversation03l_02", "s_wala_wallaconversation03l_03", "s_wala_wallaconversation03l_04", "s_wala_wallaconversation03l_05", "s_wala_wallaconversation03l_06", "s_wala_wallaconversation03l_07", "s_wala_wallaconversation03l_08", "s_wala_wallaconversation03l_09", "s_wala_wallaconversation03l_10", "s_wala_wallaconversation03l_11", "s_wala_wallaconversation03l_12", "s_wala_wallaconversation03l_13"];
    level.battlechatter.eventstrings["RU"]["idle_conv"][3] = ["s_wala_wallaconversation04l", "s_wala_wallaconversation04l_01", "s_wala_wallaconversation04l_02", "s_wala_wallaconversation04l_03", "s_wala_wallaconversation04l_04", "s_wala_wallaconversation04l_05", "s_wala_wallaconversation04l_06", "s_wala_wallaconversation04l_07", "s_wala_wallaconversation04l_08", "s_wala_wallaconversation04l_09", "s_wala_wallaconversation04l_10", "s_wala_wallaconversation04l_11", "s_wala_wallaconversation04l_12", "s_wala_wallaconversation04l_13", "s_wala_wallaconversation04l_14"];
    level.battlechatter.eventstrings["RU"]["idle_conv"][4] = ["s_wala_wallaconversation05l", "s_wala_wallaconversation05l_01", "s_wala_wallaconversation05l_02", "s_wala_wallaconversation05l_03", "s_wala_wallaconversation05l_04", "s_wala_wallaconversation05l_05", "s_wala_wallaconversation05l_06", "s_wala_wallaconversation05l_07", "s_wala_wallaconversation05l_08", "s_wala_wallaconversation05l_09"];
    level.battlechatter.eventstrings["RU"]["idle_conv"][5] = ["s_wala_wallaconversation06l", "s_wala_wallaconversation06l_01", "s_wala_wallaconversation06l_02", "s_wala_wallaconversation06l_03", "s_wala_wallaconversation06l_04", "s_wala_wallaconversation06l_05", "s_wala_wallaconversation06l_06", "s_wala_wallaconversation06l_07", "s_wala_wallaconversation06l_08", "s_wala_wallaconversation06l_09", "s_wala_wallaconversation06l_10", "s_wala_wallaconversation06l_11"];
    map_name = default_to(level.script, "");
    if (map_name == "cp_jup_port") {
        level.battlechatter.eventstrings["KON"]["idle_solo"] = [];
        level.battlechatter.eventstrings["KON"]["idle_solo"][0] = ["s_icsp_statusreport", "s_icsp_haventlocatedthepayl", "s_icsp_hesnotgonnabehappyab", "s_icsp_thecompanysgonethrou", "s_icsp_thenitshiddensomewhe", "s_icsp_keepyoureyesopen"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][1] = ["s_isp2_wedonthavetimeforthi", "s_isp2_weremarkingaswegowel", "s_isp2_howmuchhavewesearche", "s_isp2_aboutfortypercentoft", "s_isp2_onlyforty", "s_isp2_wecanbruteforceitweh"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][2] = ["s_ic03_howsyourperimeter", "s_ic03_justfinishedasweepal", "s_ic03_copy", "s_ic03_illtakeanotherpassin", "s_ic03_goodstayonit", "s_ic03_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][3] = ["s_ic35_thisistango01iminpos", "s_ic35_copyhowdoesitlook", "s_ic35_areaislocked", "s_ic35_sweepagainstayonthis", "s_ic35_copywillupdateinfive", "s_ic35_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][4] = ["s_ic23_whiskey02areweready", "s_ic23_negativeinprogress", "s_ic23_eta", "s_ic23_tenmikes", "s_ic23_movefasterclocksrunn", "s_ic23_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][5] = ["s_ic05_delta03howmuchlonger", "s_ic05_5mikes", "s_ic05_whenyouredonemoveont", "s_ic05_rogerwantmetocomebac", "s_ic05_sendcharliesquadover", "s_ic05_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][6] = ["s_ic37_thisischarlie01givem", "s_ic37_charlie01exfilinboun", "s_ic37_markingstationone", "s_ic37_affirmative", "s_ic37_beonguard", "s_ic37_willradiowheniseethe"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][7] = ["s_ic01_ineedastatusupdate", "s_ic01_stillinprogress", "s_ic01_whatstheholdup", "s_ic01_itwentoffline", "s_ic01_eta", "s_ic01_10mikes"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][8] = ["s_ic27_thisisdelta02noactiv", "s_ic27_copy02", "s_ic27_goingtofindcharliehi", "s_ic27_copystayalerthostile", "s_ic27_willradiowhenivemade", "s_ic27_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][9] = ["s_ic45_zulu02inposition", "s_ic45_copystayputcommsares", "s_ic45_contact", "s_ic45_unknownstayonguardun", "s_ic45_rogergunsareready", "s_ic45_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][10] = ["s_ic31_thisisalpha02allquie", "s_ic31_copythatalphamaintai", "s_ic31_roger", "s_ic31_deltasstill10mikesou", "s_ic31_willholduntiltheyarr", "s_ic31_beonguard"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][11] = ["s_ic43_delta01inpositionare", "s_ic43_copy01youconnectwith", "s_ic43_affirmativetheyreinp", "s_ic43_copy", "s_ic43_therearealotofteamso", "s_ic43_commandsnottakingany"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][12] = ["s_ic25_alpha03toallteamspos", "s_ic25_goodcommandsnotfucki", "s_ic25_shouldimeetwithecho", "s_ic25_negativeholdposition", "s_ic25_rogerstandingby", "s_ic25_updateinfive"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][13] = ["s_ic11_alphateamreport", "s_ic11_thisisalphawerewrapp", "s_ic11_copyyouarecleartopro", "s_ic11_whatsdeltassituation", "s_ic11_halfwaydoneonschedul", "s_ic11_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][14] = ["s_ic39_whatsyourstatustango", "s_ic39_tango02allclear", "s_ic39_copyyouseecharlie", "s_ic39_affirmativecharlieis", "s_ic39_stayalertandholdposi", "s_ic39_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][15] = ["s_ic13_tango02whenyouredone", "s_ic13_copy", "s_ic13_makeitquick", "s_ic13_didsomethinghappen", "s_ic13_captainwantstwomanfi", "s_ic13_rogertango01"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][16] = ["s_ic41_thisissierra02inposi", "s_ic41_copysierrazuluandalp", "s_ic41_eta", "s_ic41_tminusfour", "s_ic41_statusonecho", "s_ic41_movingnow"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][17] = ["s_ic09_alpha03areyoudone", "s_ic09_negativeinprogress", "s_ic09_shouldvebeenfinished", "s_ic09_itllbeonschedule", "s_ic09_pickupthepacecommand", "s_ic09_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][18] = ["s_ic19_commandthisisalpha01", "s_ic19_lastknownlocation", "s_ic19_sector7", "s_ic19_inspectitbeonguard", "s_ic19_copy", "s_ic19_givemeupdateseveryfi"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][19] = ["s_ic29_whiskey04reportingal", "s_ic29_copywhiskeykeepmeupd"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][20] = ["s_ic17_alpha03havereinforce", "s_ic17_negativewearestillwa", "s_ic17_updatemewhentheydo", "s_ic17_copystillonschedule", "s_ic17_notiftangodoesntgeth", "s_ic17_illmakesuretheydo"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][21] = ["s_ic07_alpha02anyhostiles", "s_ic07_negativenothingyet", "s_ic07_copymaintainposition", "s_ic07_youseeanything", "s_ic07_negativebutstayalert", "s_ic07_rogerthatalpha01"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][22] = ["s_ic21_delta02haveyoufounda", "s_ic21_negativeandyou", "s_ic21_nothingyet", "s_ic21_wasourinteloff", "s_ic21_keepsearchingitshere", "s_ic21_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][23] = ["s_ic15_alphateamreport", "s_ic15_inpositionnomovement", "s_ic15_copystayonalert", "s_ic15_anythingfromtheother", "s_ic15_negativenocontacts", "s_ic15_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][24] = ["s_ic33_rotationcommencingin", "s_ic33_copywhostakingover", "s_ic33_delta", "s_ic33_canyouconfirmournext", "s_ic33_affirmativesector3no", "s_ic33_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"] = [];
        level.battlechatter.eventstrings["KON"]["idle_conv"][0] = ["s_id01_perimeterclear", "s_id01_affirmativenoactivit", "s_id01_theambushwaseffectiv", "s_id01_charliesstillmopping", "s_id01_tellthemtohurryuplet", "s_id01_copythat"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][1] = ["s_icdf_youfoundherfarahkari", "s_icdf_onlythebodyoftheharb", "s_icdf_wastheshipsmanifesto", "s_icdf_negative", "s_icdf_thenwellhavetodothis", "s_icdf_gotateamontheboatnow"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][2] = ["s_ic04_anyactivity", "s_ic04_perimetersclear", "s_ic04_sweepitagain", "s_ic04_copywillheadoutinfiv", "s_ic04_staysharp", "s_ic04_rog"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][3] = ["s_ic36_sierrasinposition", "s_ic36_zulusontheperimeter", "s_ic36_radiocommandtellthem", "s_ic36_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][4] = ["s_ic24_youhaveenoughcharges", "s_ic24_negativerunninglow", "s_ic24_locatedeltaforresupp", "s_ic24_copy", "s_ic24_needtobedoneinfive", "s_ic24_onit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][5] = ["s_ic06_eta", "s_ic06_shouldbedonein20", "s_ic06_hopedforsooner", "s_ic06_whatmattersisthatits", "s_ic06_captainstimelinenotm", "s_ic06_wereonit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][6] = ["s_ic38_whatsthestatusonexfi", "s_ic38_commandsayshelosared", "s_ic38_howmany", "s_ic38_three", "s_ic38_copyholdyourposition", "s_ic38_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][7] = ["s_ic02_whatsyourstatus", "s_ic02_workingonit", "s_ic02_helosarealreadyinbou", "s_ic02_illbringintangotohel", "s_ic02_getitdonein15", "s_ic02_willdoitin10"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][8] = ["s_ic28_sector2isclear", "s_ic28_wehavereportsofhosti", "s_ic28_yessir", "s_ic28_youseeanythingradioa", "s_ic28_copy", "s_ic28_switchinfourhours"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][9] = ["s_ic46_commandgavewordtomov", "s_ic46_arewecoveringtheirse", "s_ic46_negativetheyrehandle", "s_ic46_copywelldoanotherswe", "s_ic46_agreedwellsplitthete", "s_ic46_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][10] = ["s_ic32_cantseeshit", "s_ic32_wellholdhere", "s_ic32_alphassweepingsector", "s_ic32_keepwatchiftheymoves", "s_ic32_roger", "s_ic32_illupdatecommand"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][11] = ["s_ic44_weset", "s_ic44_affirmative", "s_ic44_status", "s_ic44_allclear", "s_ic44_stayvigilantcheckini", "s_ic44_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][12] = ["s_ic26_commandwantseverythi", "s_ic26_charliesdoinganother", "s_ic26_makesuretheyrethorou", "s_ic26_roger", "s_ic26_nowitnesses", "s_ic26_willreportbackwhenwe"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][13] = ["s_ic12_captainwantsanupdate", "s_ic12_tenmoreminutes", "s_ic12_makeitinfive", "s_ic12_weneedmorebodiesonit", "s_ic12_charlieteam", "s_ic12_theyshouldbewrappeds"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][14] = ["s_ic40_commandwantstheaoloc", "s_ic40_onit", "s_ic40_keepthosecornersclea", "s_ic40_anythinginbound", "s_ic40_nothingyetordersaret", "s_ic40_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][15] = ["s_ic14_wereceiveneworders", "s_ic14_affirmativeweretomee", "s_ic14_location", "s_ic14_outerperimeterfullsw", "s_ic14_copy", "s_ic14_letsmove"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][16] = ["s_ic42_whiskeyisinposition", "s_ic42_zulu", "s_ic42_inboundinfive", "s_ic42_theyrelate", "s_ic42_charliewillcover", "s_ic42_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][17] = ["s_ic10_howarewedoingonprep", "s_ic10_everythingisset", "s_ic10_sector2", "s_ic10_tangoteamsgotitlocke", "s_ic10_wewillmeetwiththemon", "s_ic10_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][18] = ["s_ic20_deltamissedacommchec", "s_ic20_whatwastheirlastloca", "s_ic20_patrollingthefarside", "s_ic20_notifytangotheyreclo", "s_ic20_shouldwejoin", "s_ic20_negativestayonpost"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][19] = ["s_ic30_whiskeysquadscombing", "s_ic30_andecho", "s_ic30_patrolling", "s_ic30_copybereadytomove", "s_ic30_yessir", "s_ic30_untilthenbevigilant"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][20] = ["s_ic18_commandsentanupdater", "s_ic18_gooddeltaneedsmoresu", "s_ic18_andzulu", "s_ic18_finishedtenminutesag", "s_ic18_tellthemtosupportdel", "s_ic18_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][21] = ["s_ic08_anymovement", "s_ic08_negativeyou", "s_ic08_notyet", "s_ic08_hostilescouldbeplayi", "s_ic08_ingoodpositionwerese", "s_ic08_copythatkeepingwatch"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][22] = ["s_ic22_anyupdates", "s_ic22_negative", "s_ic22_itsgoingtobearoundhe", "s_ic22_permissiontoredirect", "s_ic22_negativealphasonpatr", "s_ic22_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][23] = ["s_ic16_wherearetheothers", "s_ic16_alpha02and03aresecur", "s_ic16_andthecaptain", "s_ic16_syncingwithdeltateam", "s_ic16_justyouandmethen", "s_ic16_morethanenough"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][24] = ["s_ic34_deltasshiftingtosect", "s_ic34_whythechange", "s_ic34_alphasquadarrived", "s_ic34_copy", "s_ic34_weremorethancoveredn", "s_ic34_letsgetamoveon"];
    }
    if (map_name == "cp_jup_chemical") {
        level.battlechatter.eventstrings["KON"]["idle_solo"] = [];
        level.battlechatter.eventstrings["KON"]["idle_solo"][0] = ["s_icsc_anysignoftheulf", "s_icsc_negative", "s_icsc_thatcanchangegotword", "s_icsc_ifulfstheretheyrenot", "s_icsc_juststayalert", "s_icsc_copythat"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][1] = ["s_ic03_howsyourperimeter", "s_ic03_justfinishedasweepal", "s_ic03_copy", "s_ic03_illtakeanotherpassin", "s_ic03_goodstayonit", "s_ic03_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][2] = ["s_ic35_thisistango01iminpos", "s_ic35_copyhowdoesitlook", "s_ic35_areaislocked", "s_ic35_sweepagainstayonthis", "s_ic35_copywillupdateinfive", "s_ic35_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][3] = ["s_ic23_whiskey02areweready", "s_ic23_negativeinprogress", "s_ic23_eta", "s_ic23_tenmikes", "s_ic23_movefasterclocksrunn", "s_ic23_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][4] = ["s_ic05_delta03howmuchlonger", "s_ic05_5mikes", "s_ic05_whenyouredonemoveont", "s_ic05_rogerwantmetocomebac", "s_ic05_sendcharliesquadover", "s_ic05_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][5] = ["s_ic37_thisischarlie01givem", "s_ic37_charlie01exfilinboun", "s_ic37_markingstationone", "s_ic37_affirmative", "s_ic37_beonguard", "s_ic37_willradiowheniseethe"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][6] = ["s_ic01_ineedastatusupdate", "s_ic01_stillinprogress", "s_ic01_whatstheholdup", "s_ic01_itwentoffline", "s_ic01_eta", "s_ic01_10mikes"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][7] = ["s_ic27_thisisdelta02noactiv", "s_ic27_copy02", "s_ic27_goingtofindcharliehi", "s_ic27_copystayalerthostile", "s_ic27_willradiowhenivemade", "s_ic27_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][8] = ["s_ic45_zulu02inposition", "s_ic45_copystayputcommsares", "s_ic45_contact", "s_ic45_unknownstayonguardun", "s_ic45_rogergunsareready", "s_ic45_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][9] = ["s_ic31_thisisalpha02allquie", "s_ic31_copythatalphamaintai", "s_ic31_roger", "s_ic31_deltasstill10mikesou", "s_ic31_willholduntiltheyarr", "s_ic31_beonguard"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][10] = ["s_ic43_delta01inpositionare", "s_ic43_copy01youconnectwith", "s_ic43_affirmativetheyreinp", "s_ic43_copy", "s_ic43_therearealotofteamso", "s_ic43_commandsnottakingany"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][11] = ["s_ic25_alpha03toallteamspos", "s_ic25_goodcommandsnotfucki", "s_ic25_shouldimeetwithecho", "s_ic25_negativeholdposition", "s_ic25_rogerstandingby", "s_ic25_updateinfive"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][12] = ["s_ic11_alphateamreport", "s_ic11_thisisalphawerewrapp", "s_ic11_copyyouarecleartopro", "s_ic11_whatsdeltassituation", "s_ic11_halfwaydoneonschedul", "s_ic11_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][13] = ["s_ic39_whatsyourstatustango", "s_ic39_tango02allclear", "s_ic39_copyyouseecharlie", "s_ic39_affirmativecharlieis", "s_ic39_stayalertandholdposi", "s_ic39_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][14] = ["s_ic13_tango02whenyouredone", "s_ic13_copy", "s_ic13_makeitquick", "s_ic13_didsomethinghappen", "s_ic13_captainwantstwomanfi", "s_ic13_rogertango01"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][15] = ["s_ic41_thisissierra02inposi", "s_ic41_copysierrazuluandalp", "s_ic41_eta", "s_ic41_tminusfour", "s_ic41_statusonecho", "s_ic41_movingnow"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][16] = ["s_ic09_alpha03areyoudone", "s_ic09_negativeinprogress", "s_ic09_shouldvebeenfinished", "s_ic09_itllbeonschedule", "s_ic09_pickupthepacecommand", "s_ic09_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][17] = ["s_ic19_commandthisisalpha01", "s_ic19_lastknownlocation", "s_ic19_sector7", "s_ic19_inspectitbeonguard", "s_ic19_copy", "s_ic19_givemeupdateseveryfi"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][18] = ["s_ic29_whiskey04reportingal", "s_ic29_copywhiskeykeepmeupd"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][19] = ["s_ic17_alpha03havereinforce", "s_ic17_negativewearestillwa", "s_ic17_updatemewhentheydo", "s_ic17_copystillonschedule", "s_ic17_notiftangodoesntgeth", "s_ic17_illmakesuretheydo"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][20] = ["s_ic07_alpha02anyhostiles", "s_ic07_negativenothingyet", "s_ic07_copymaintainposition", "s_ic07_youseeanything", "s_ic07_negativebutstayalert", "s_ic07_rogerthatalpha01"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][21] = ["s_ic21_delta02haveyoufounda", "s_ic21_negativeandyou", "s_ic21_nothingyet", "s_ic21_wasourinteloff", "s_ic21_keepsearchingitshere", "s_ic21_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][22] = ["s_ic15_alphateamreport", "s_ic15_inpositionnomovement", "s_ic15_copystayonalert", "s_ic15_anythingfromtheother", "s_ic15_negativenocontacts", "s_ic15_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][23] = ["s_ic33_rotationcommencingin", "s_ic33_copywhostakingover", "s_ic33_delta", "s_ic33_canyouconfirmournext", "s_ic33_affirmativesector3no", "s_ic33_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"] = [];
        level.battlechatter.eventstrings["KON"]["idle_conv"][0] = ["s_ichg_wefindthatpayloadwen", "s_ichg_alphasquadsonit", "s_ichg_fasterwefindthepaylo", "s_ichg_illradiocharlieseeif", "s_ichg_gotcheckinseveryfive", "s_ichg_copythat"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][1] = ["s_ic04_anyactivity", "s_ic04_perimetersclear", "s_ic04_sweepitagain", "s_ic04_copywillheadoutinfiv", "s_ic04_staysharp", "s_ic04_rog"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][2] = ["s_ic36_sierrasinposition", "s_ic36_zulusontheperimeter", "s_ic36_radiocommandtellthem", "s_ic36_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][3] = ["s_ic24_youhaveenoughcharges", "s_ic24_negativerunninglow", "s_ic24_locatedeltaforresupp", "s_ic24_copy", "s_ic24_needtobedoneinfive", "s_ic24_onit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][4] = ["s_ic06_eta", "s_ic06_shouldbedonein20", "s_ic06_hopedforsooner", "s_ic06_whatmattersisthatits", "s_ic06_captainstimelinenotm", "s_ic06_wereonit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][5] = ["s_ic38_whatsthestatusonexfi", "s_ic38_commandsayshelosared", "s_ic38_howmany", "s_ic38_three", "s_ic38_copyholdyourposition", "s_ic38_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][6] = ["s_ic02_whatsyourstatus", "s_ic02_workingonit", "s_ic02_helosarealreadyinbou", "s_ic02_illbringintangotohel", "s_ic02_getitdonein15", "s_ic02_willdoitin10"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][7] = ["s_ic28_sector2isclear", "s_ic28_wehavereportsofhosti", "s_ic28_yessir", "s_ic28_youseeanythingradioa", "s_ic28_copy", "s_ic28_switchinfourhours"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][8] = ["s_ic46_commandgavewordtomov", "s_ic46_arewecoveringtheirse", "s_ic46_negativetheyrehandle", "s_ic46_copywelldoanotherswe", "s_ic46_agreedwellsplitthete", "s_ic46_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][9] = ["s_ic32_cantseeshit", "s_ic32_wellholdhere", "s_ic32_alphassweepingsector", "s_ic32_keepwatchiftheymoves", "s_ic32_roger", "s_ic32_illupdatecommand"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][10] = ["s_ic44_weset", "s_ic44_affirmative", "s_ic44_status", "s_ic44_allclear", "s_ic44_stayvigilantcheckini", "s_ic44_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][11] = ["s_ic26_commandwantseverythi", "s_ic26_charliesdoinganother", "s_ic26_makesuretheyrethorou", "s_ic26_roger", "s_ic26_nowitnesses", "s_ic26_willreportbackwhenwe"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][12] = ["s_ic12_captainwantsanupdate", "s_ic12_tenmoreminutes", "s_ic12_makeitinfive", "s_ic12_weneedmorebodiesonit", "s_ic12_charlieteam", "s_ic12_theyshouldbewrappeds"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][13] = ["s_ic40_commandwantstheaoloc", "s_ic40_onit", "s_ic40_keepthosecornersclea", "s_ic40_anythinginbound", "s_ic40_nothingyetordersaret", "s_ic40_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][14] = ["s_ic14_wereceiveneworders", "s_ic14_affirmativeweretomee", "s_ic14_location", "s_ic14_outerperimeterfullsw", "s_ic14_copy", "s_ic14_letsmove"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][15] = ["s_ic42_whiskeyisinposition", "s_ic42_zulu", "s_ic42_inboundinfive", "s_ic42_theyrelate", "s_ic42_charliewillcover", "s_ic42_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][16] = ["s_ic10_howarewedoingonprep", "s_ic10_everythingisset", "s_ic10_sector2", "s_ic10_tangoteamsgotitlocke", "s_ic10_wewillmeetwiththemon", "s_ic10_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][17] = ["s_ic20_deltamissedacommchec", "s_ic20_whatwastheirlastloca", "s_ic20_patrollingthefarside", "s_ic20_notifytangotheyreclo", "s_ic20_shouldwejoin", "s_ic20_negativestayonpost"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][19] = ["s_ic30_whiskeysquadscombing", "s_ic30_andecho", "s_ic30_patrolling", "s_ic30_copybereadytomove", "s_ic30_yessir", "s_ic30_untilthenbevigilant"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][19] = ["s_ic18_commandsentanupdater", "s_ic18_gooddeltaneedsmoresu", "s_ic18_andzulu", "s_ic18_finishedtenminutesag", "s_ic18_tellthemtosupportdel", "s_ic18_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][20] = ["s_ic08_anymovement", "s_ic08_negativeyou", "s_ic08_notyet", "s_ic08_hostilescouldbeplayi", "s_ic08_ingoodpositionwerese", "s_ic08_copythatkeepingwatch"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][21] = ["s_ic22_anyupdates", "s_ic22_negative", "s_ic22_itsgoingtobearoundhe", "s_ic22_permissiontoredirect", "s_ic22_negativealphasonpatr", "s_ic22_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][22] = ["s_ic16_wherearetheothers", "s_ic16_alpha02and03aresecur", "s_ic16_andthecaptain", "s_ic16_syncingwithdeltateam", "s_ic16_justyouandmethen", "s_ic16_morethanenough"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][23] = ["s_ic34_deltasshiftingtosect", "s_ic34_whythechange", "s_ic34_alphasquadarrived", "s_ic34_copy", "s_ic34_weremorethancoveredn", "s_ic34_letsgetamoveon"];
    }
    if (map_name == "cp_jup_ranch") {
        level.battlechatter.eventstrings["KON"]["idle_solo"] = [];
        level.battlechatter.eventstrings["KON"]["idle_solo"][0] = ["s_icsr_delta02toalpha01what", "s_icsr_delta02perimeterclea", "s_icsr_copykeepitlockeddown", "s_icsr_isalphadonewiththebo", "s_icsr_almostwillkeepyouupd", "s_icsr_rogerthat"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][1] = ["s_isr2_locatedanything", "s_isr2_foundacouplecellsnea", "s_isr2_ifanythingsurvivedwe", "s_isr2_illdoanotherlooparou", "s_isr2_reportbackinfive", "s_isr2_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][2] = ["s_ic03_howsyourperimeter", "s_ic03_justfinishedasweepal", "s_ic03_copy", "s_ic03_illtakeanotherpassin", "s_ic03_goodstayonit", "s_ic03_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][3] = ["s_ic35_thisistango01iminpos", "s_ic35_copyhowdoesitlook", "s_ic35_areaislocked", "s_ic35_sweepagainstayonthis", "s_ic35_copywillupdateinfive", "s_ic35_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][4] = ["s_ic23_whiskey02areweready", "s_ic23_negativeinprogress", "s_ic23_eta", "s_ic23_tenmikes", "s_ic23_movefasterclocksrunn", "s_ic23_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][5] = ["s_ic05_delta03howmuchlonger", "s_ic05_5mikes", "s_ic05_whenyouredonemoveont", "s_ic05_rogerwantmetocomebac", "s_ic05_sendcharliesquadover", "s_ic05_roger"];
        ...
        level.battlechatter.eventstrings["KON"]["idle_solo"][24] = ["s_ic33_rotationcommencingin", "s_ic33_copywhostakingover", "s_ic33_delta", "s_ic33_canyouconfirmournext", "s_ic33_affirmativesector3no", "s_ic33_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"] = [];
        level.battlechatter.eventstrings["KON"]["idle_conv"][0] = ["s_icdr_findanything", "s_icdr_negativejustdebrisyo", "s_icdr_notyet", "s_icdr_searchteamsaremaking", "s_icdr_thatjustleavesthecel", "s_icdr_wefindanyfootagethat"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][1] = ["s_icso_seeanyphonesoverther", "s_icso_negativemovingontoth", "s_icso_searchteamspulledthe", "s_icso_goodanythingelsefrom", "s_icso_tightenperimeter", "s_icso_noonewillgetinoroutu"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][2] = ["s_icut_notfindingmuchjustde", "s_icut_keepsearchingbosswan", "s_icut_phonescameras", "s_icut_anythingthatsnotours", "s_icut_roger", "s_icut_wefindanythingthatim"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][3] = ["s_ic04_anyactivity", "s_ic04_perimetersclear", "s_ic04_sweepitagain", "s_ic04_copywillheadoutinfiv", "s_ic04_staysharp", "s_ic04_rog"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][4] = ["s_ic36_sierrasinposition", "s_ic36_zulusontheperimeter", "s_ic36_radiocommandtellthem", "s_ic36_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][5] = ["s_ic24_youhaveenoughcharges", "s_ic24_negativerunninglow", "s_ic24_locatedeltaforresupp", "s_ic24_copy", "s_ic24_needtobedoneinfive", "s_ic24_onit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][6] = ["s_ic06_eta", "s_ic06_shouldbedonein20", "s_ic06_hopedforsooner", "s_ic06_whatmattersisthatits", "s_ic06_captainstimelinenotm", "s_ic06_wereonit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][7] = ["s_ic38_whatsthestatusonexfi", "s_ic38_commandsayshelosared", "s_ic38_howmany", "s_ic38_three", "s_ic38_copyholdyourposition", "s_ic38_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][8] = ["s_ic02_whatsyourstatus", "s_ic02_workingonit", "s_ic02_helosarealreadyinbou", "s_ic02_illbringintangotohel", "s_ic02_getitdonein15", "s_ic02_willdoitin10"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][9] = ["s_ic28_sector2isclear", "s_ic28_wehavereportsofhosti", "s_ic28_yessir", "s_ic28_youseeanythingradioa", "s_ic28_copy", "s_ic28_switchinfourhours"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][10] = ["s_ic46_commandgavewordtomov", "s_ic46_arewecoveringtheirse", "s_ic46_negativetheyrehandle", "s_ic46_copywelldoanotherswe", "s_ic46_agreedwellsplitthete", "s_ic46_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][11] = ["s_ic32_cantseeshit", "s_ic32_wellholdhere", "s_ic32_alphassweepingsector", "s_ic32_keepwatchiftheymoves", "s_ic32_roger", "s_ic32_illupdatecommand"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][12] = ["s_ic44_weset", "s_ic44_affirmative", "s_ic44_status", "s_ic44_allclear", "s_ic44_stayvigilantcheckini", "s_ic44_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][13] = ["s_ic26_commandwantseverythi", "s_ic26_charliesdoinganother", "s_ic26_makesuretheyrethorou", "s_ic26_roger", "s_ic26_nowitnesses", "s_ic26_willreportbackwhenwe"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][14] = ["s_ic12_captainwantsanupdate", "s_ic12_tenmoreminutes", "s_ic12_makeitinfive", "s_ic12_weneedmorebodiesonit", "s_ic12_charlieteam", "s_ic12_theyshouldbewrappeds"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][15] = ["s_ic40_commandwantstheaoloc", "s_ic40_onit", "s_ic40_keepthosecornersclea", "s_ic40_anythinginbound", "s_ic40_nothingyetordersaret", "s_ic40_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][16] = ["s_ic14_wereceiveneworders", "s_ic14_affirmativeweretomee", "s_ic14_location", "s_ic14_outerperimeterfullsw", "s_ic14_copy", "s_ic14_letsmove"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][17] = ["s_ic42_whiskeyisinposition", "s_ic42_zulu", "s_ic42_inboundinfive", "s_ic42_theyrelate", "s_ic42_charliewillcover", "s_ic42_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][18] = ["s_ic10_howarewedoingonprep", "s_ic10_everythingisset", "s_ic10_sector2", "s_ic10_tangoteamsgotitlocke", "s_ic10_wewillmeetwiththemon", "s_ic10_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][19] = ["s_ic20_deltamissedacommchec", "s_ic20_whatwastheirlastloca", "s_ic20_patrollingthefarside", "s_ic20_notifytangotheyreclo", "s_ic20_shouldwejoin", "s_ic20_negativestayonpost"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][20] = ["s_ic30_whiskeysquadscombing", "s_ic30_andecho", "s_ic30_patrolling", "s_ic30_copybereadytomove", "s_ic30_yessir", "s_ic30_untilthenbevigilant"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][21] = ["s_ic18_commandsentanupdater", "s_ic18_gooddeltaneedsmoresu", "s_ic18_andzulu", "s_ic18_finishedtenminutesag", "s_ic18_tellthemtosupportdel", "s_ic18_copy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][22] = ["s_ic08_anymovement", "s_ic08_negativeyou", "s_ic08_notyet", "s_ic08_hostilescouldbeplayi", "s_ic08_ingoodpositionwerese", "s_ic08_copythatkeepingwatch"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][23] = ["s_ic22_anyupdates", "s_ic22_negative", "s_ic22_itsgoingtobearoundhe", "s_ic22_permissiontoredirect", "s_ic22_negativealphasonpatr", "s_ic22_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][24] = ["s_ic16_wherearetheothers", "s_ic16_alpha02and03aresecur", "s_ic16_andthecaptain", "s_ic16_syncingwithdeltateam", "s_ic16_justyouandmethen", "s_ic16_morethanenough"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][25] = ["s_ic34_deltasshiftingtosect", "s_ic34_whythechange", "s_ic34_alphasquadarrived", "s_ic34_copy", "s_ic34_weremorethancoveredn", "s_ic34_letsgetamoveon"];
    }
    if (map_name == "cp_jup_resort") {
        level.battlechatter.eventstrings["KON"]["idle_solo"] = [];
        level.battlechatter.eventstrings["KON"]["idle_solo"][0] = ["s_icst_weshoulddoanothersea", "s_icst_romanovawantsanupdat", "s_icst_everyfiveminutes", "s_icst_sheknowswhatshesdoin", "s_icst_tellcharlieanddeltat", "s_icst_copythatrelaying"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][1] = ["s_ic03_howsyourperimeter", "s_ic03_justfinishedasweepal", "s_ic03_copy", "s_ic03_illtakeanotherpassin", "s_ic03_goodstayonit", "s_ic03_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][2] = ["s_ic35_thisistango01iminpos", "s_ic35_copyhowdoesitlook", "s_ic35_areaislocked", "s_ic35_sweepagainstayonthis", "s_ic35_copywillupdateinfive", "s_ic35_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][3] = ["s_ic23_whiskey02areweready", "s_ic23_negativeinprogress", "s_ic23_eta", "s_ic23_tenmikes", "s_ic23_movefasterclocksrunn", "s_ic23_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][4] = ["s_ic05_delta03howmuchlonger", "s_ic05_5mikes", "s_ic05_whenyouredonemoveont", "s_ic05_rogerwantmetocomebac", "s_ic05_sendcharliesquadover", "s_ic05_roger"];
        ...
        level.battlechatter.eventstrings["KON"]["idle_solo"][23] = ["s_ic33_rotationcommencingin", "s_ic33_copywhostakingover", "s_ic33_delta", "s_ic33_canyouconfirmournext", "s_ic33_affirmativesector3no", "s_ic33_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"] = [];
        level.battlechatter.eventstrings["KON"]["idle_conv"][0] = ["s_icor_romanovasbeengetting", "s_icor_deltasmonitoringfori", "s_icor_thingsaregettinghot", "s_icor_wellneedtotightensec", "s_icor_intruderslikelythen", "s_icor_affirmativesweepthea"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][1] = ["s_icsf_charlieanddeltasquad", "s_icsf_romanovawantsupdates", "s_icsf_copythatillletthemkn", "s_icsf_anynewsfromtheothers", "s_icsf_theyreclear", "s_icsf_tellthemtocheckagain"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][2] = ["s_ic04_anyactivity", "s_ic04_perimetersclear", "s_ic04_sweepitagain", "s_ic04_copywillheadoutinfiv", "s_ic04_staysharp", "s_ic04_rog"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][3] = ["s_ic36_sierrasinposition", "s_ic36_zulusontheperimeter", "s_ic36_radiocommandtellthem", "s_ic36_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][4] = ["s_ic24_youhaveenoughcharges", "s_ic24_negativerunninglow", "s_ic24_locatedeltaforresupp", "s_ic24_copy", "s_ic24_needtobedoneinfive", "s_ic24_onit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][5] = ["s_ic06_eta", "s_ic06_shouldbedonein20", "s_ic06_hopedforsooner", "s_ic06_whatmattersisthatits", "s_ic06_captainstimelinenotm", "s_ic06_wereonit"];
    ...
    if (map_name == "cp_jup_dam") {
        level.battlechatter.eventstrings["KON"]["idle_solo"] = [];
        level.battlechatter.eventstrings["KON"]["idle_solo"][0] = ["s_icsd_charlie04thisisalpha", "s_icsd_payloadssetperimeter", "s_icsd_checkitagaincantbeto", "s_icsd_youveseenthechemical", "s_icsd_lethalashelldontdrop", "s_icsd_solidcopy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][1] = ["s_ic03_howsyourperimeter", "s_ic03_justfinishedasweepal", "s_ic03_copy", "s_ic03_illtakeanotherpassin", "s_ic03_goodstayonit", "s_ic03_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][2] = ["s_ic35_thisistango01iminpos", "s_ic35_copyhowdoesitlook", "s_ic35_areaislocked", "s_ic35_sweepagainstayonthis", "s_ic35_copywillupdateinfive", "s_ic35_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][3] = ["s_ic23_whiskey02areweready", "s_ic23_negativeinprogress", "s_ic23_eta", "s_ic23_tenmikes", "s_ic23_movefasterclocksrunn", "s_ic23_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][4] = ["s_ic05_delta03howmuchlonger", "s_ic05_5mikes", "s_ic05_whenyouredonemoveont", "s_ic05_rogerwantmetocomebac", "s_ic05_sendcharliesquadover", "s_ic05_roger"];
        ...
        level.battlechatter.eventstrings["KON"]["idle_solo"][23] = ["s_ic33_rotationcommencingin", "s_ic33_copywhostakingover", "s_ic33_delta", "s_ic33_canyouconfirmournext", "s_ic33_affirmativesector3no", "s_ic33_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"] = [];
        level.battlechatter.eventstrings["KON"]["idle_conv"][0] = ["s_icpa_anyupdatesfromteam4", "s_icpa_theirpayloadsalmosts", "s_icpa_tellthemtohurryitup", "s_icpa_copy", "s_icpa_havethesnipersholdon", "s_icpa_theyrealreadyinposit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][1] = ["s_id03_charlieteamsaysthepa", "s_id03_tellthemtostayalertd", "s_id03_copythat", "s_id03_thenwegottamove", "s_id03_jltvsarestandingby", "s_id03_dontwanttobeanywhere"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][2] = ["s_ic04_anyactivity", "s_ic04_perimetersclear", "s_ic04_sweepitagain", "s_ic04_copywillheadoutinfiv", "s_ic04_staysharp", "s_ic04_rog"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][3] = ["s_ic36_sierrasinposition", "s_ic36_zulusontheperimeter", "s_ic36_radiocommandtellthem", "s_ic36_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][4] = ["s_ic24_youhaveenoughcharges", "s_ic24_negativerunninglow", "s_ic24_locatedeltaforresupp", "s_ic24_copy", "s_ic24_needtobedoneinfive", "s_ic24_onit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][5] = ["s_ic06_eta", "s_ic06_shouldbedonein20", "s_ic06_hopedforsooner", "s_ic06_whatmattersisthatits", "s_ic06_captainstimelinenotm", "s_ic06_wereonit"];
    ...
    if (map_name == "cp_jup_apt") {
        level.battlechatter.eventstrings["KON"]["idle_solo"] = [];
        level.battlechatter.eventstrings["KON"]["idle_solo"][0] = ["s_ic47_delta04status", "s_ic47_shipmentsnearlyready", "s_ic47_notraces", "s_ic47_wewereneverhere", "s_ic47_illupdatenolanlethim", "s_ic47_copythat"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][1] = ["s_ic03_howsyourperimeter", "s_ic03_justfinishedasweepal", "s_ic03_copy", "s_ic03_illtakeanotherpassin", "s_ic03_goodstayonit", "s_ic03_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][2] = ["s_ic35_thisistango01iminpos", "s_ic35_copyhowdoesitlook", "s_ic35_areaislocked", "s_ic35_sweepagainstayonthis", "s_ic35_copywillupdateinfive", "s_ic35_copy"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][3] = ["s_ic23_whiskey02areweready", "s_ic23_negativeinprogress", "s_ic23_eta", "s_ic23_tenmikes", "s_ic23_movefasterclocksrunn", "s_ic23_roger"];
        level.battlechatter.eventstrings["KON"]["idle_solo"][4] = ["s_ic05_delta03howmuchlonger", "s_ic05_5mikes", "s_ic05_whenyouredonemoveont", "s_ic05_rogerwantmetocomebac", "s_ic05_sendcharliesquadover", "s_ic05_roger"];
        ...
        level.battlechatter.eventstrings["KON"]["idle_solo"][23] = ["s_ic33_rotationcommencingin", "s_ic33_copywhostakingover", "s_ic33_delta", "s_ic33_canyouconfirmournext", "s_ic33_affirmativesector3no", "s_ic33_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"] = [];
        level.battlechatter.eventstrings["KON"]["idle_conv"][0] = ["s_ic49_tangoandcharliedeplo", "s_ic49_wheretheyheaded", "s_ic49_easternurzikstan", "s_ic49_whataboutus", "s_ic49_willgetourdestinatio", "s_ic49_solidcopy"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][1] = ["s_ic48_pingdeltacommandwant", "s_ic48_truckswerelatetheyre", "s_ic48_nolansnotgoingtolike", "s_ic48_illtellthemtopickupt", "s_ic48_goodtheyreexpectingt", "s_ic48_copythat"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][2] = ["s_ic04_anyactivity", "s_ic04_perimetersclear", "s_ic04_sweepitagain", "s_ic04_copywillheadoutinfiv", "s_ic04_staysharp", "s_ic04_rog"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][3] = ["s_ic36_sierrasinposition", "s_ic36_zulusontheperimeter", "s_ic36_radiocommandtellthem", "s_ic36_roger"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][4] = ["s_ic24_youhaveenoughcharges", "s_ic24_negativerunninglow", "s_ic24_locatedeltaforresupp", "s_ic24_copy", "s_ic24_needtobedoneinfive", "s_ic24_onit"];
        level.battlechatter.eventstrings["KON"]["idle_conv"][5] = ["s_ic06_eta", "s_ic06_shouldbedonein20", "s_ic06_hopedforsooner", "s_ic06_whatmattersisthatits", "s_ic06_captainstimelinenotm", "s_ic06_wereonit"];
    ...
    level.battlechatter.eventstrings["RU"]["high"] = ["c_posn_uptop", "c_posn_checkhigh"];
    ...
    level.battlechatter.eventstrings["RU"]["use_rpg"] = ["c_ator_hitemwitharocket", "c_ator_usearocket", "c_ator_rocket"];
    level.battlechatter.eventstrings["RU"]["order_attack"] = ["c_ator_engage", "c_ator_giveemsome", "c_ator_dumpem", "c_ator_gohot", "c_ator_fuckinshootem", "c_ator_startsmokinem"];
    ...
    level.battlechatter.eventstrings["RU"]["enemy_attacking"] = ["c_eata_takingfire", "c_eata_contact", "c_eata_shadowsincontact", "s_unrc_takingcontact", "s_unrc_contactcontact", "s_unrc_takingindirectfire"];
    ...
    level.battlechatter.eventstrings["RU"]["enemy_reloading"] = ["c_etca_onesreloading", "c_etca_onesreloading_01", "c_etca_oneschangingmags"];
    ...
    level.battlechatter.eventstrings["RU"]["target_moving_up"] = ["c_ttca_theyremovingup", "c_ttca_hescominguponyou", "c_ttca_hesmovingup"];
    ...
    level.battlechatter.eventstrings["RU"]["copy"] = ["c_conf_copy", "c_conf_check", "c_conf_copythat", "c_conf_goodcopy", "c_conf_solidcopy", "c_aqcp_copy", "c_aqcp_solidcopy"];
    ...
    level.battlechatter.eventstrings["RU"]["wounded_target"] = ["c_atas_woundedhim", "c_atas_ihithim", "c_atas_itaggedem", "c_tast_heshit", "c_tast_heswounded", "c_tast_hesbleeding"];
    ...
    level.battlechatter.eventstrings["RU"]["suppressing"] = ["c_aqsp_suppressingfire", "c_aqsp_imcoveringyou", "c_aqsp_coveringfire"];
    ...
    level.battlechatter.eventstrings["RU"]["resp_patrol"] = ["s_idrp_thisisthresher", "s_ahrp_thisisthresher"];
    ...
    level.battlechatter.eventstrings["RU"]["react_found_corpse"] = ["s_alrc_wegotamandown", "s_alrc_wegotacasualty", "s_alrc_gotashadowdown", "s_alrc_wegotakia"];
    level.battlechatter.eventstrings["RU"]["ask_corpse_name"] = ["s_alrc_whoisit", "s_alrc_who", "s_alrc_whodidwelose"];
    level.battlechatter.eventstrings["RU"]["need_backup"] = ["s_alrc_sendshadows", "s_alrc_sendreinforcements", "s_albc_sendreinforcements"];
    level.battlechatter.eventstrings["RU"]["hunt_copy"] = ["s_hnrs_copy", "s_hnrs_rogerthat", "s_hnrs_copythat", "s_hnrs_rog"];
    ...
    level.battlechatter.eventstrings["RU"]["inv_ask_first"] = ["s_idia_whatwasit", "s_idia_whatdyoufind", "s_idia_yougotanything", "s_idia_seeanything", "s_idia_findanything", "s_alia_seeanything", "s_alia_yougotsomething", "s_alia_anything", "s_alia_gotanything"];
    ...
    level.battlechatter.eventstrings["RU"]["enemy_loc_resp"] = ["s_unrs_idontknowbutthatwasa", "s_unrs_ididntseeem", "s_unrs_idontfuckingknow", "s_unrs_novisualdidntseeem"];
    level.battlechatter.eventstrings["RU"]["ask_enemy_count"] = ["s_unwc_howmanyarethere", "s_unwc_howmanyofem", "s_unwc_howmany"];
    level.battlechatter.eventstrings["RU"]["unaware_backup"] = ["s_unwb_ineedreinforcements", "s_unwb_getmoreshadowshereno", "s_unwb_sendreinforcementsmy"];
    level.battlechatter.eventstrings["RU"]["react_first_combat"] = ["s_awrr_contact", "s_awrr_troopsincontact", "s_awrr_enemymyposition", "s_awrr_enemy", "s_awrr_enemyshere", "s_awrr_gotenemyhere", "s_awrr_enemysclose"];
    ...
    level.battlechatter.eventstrings["RU"]["found_you"] = ["s_awrr_motherfucker", "s_awrr_foundyou", "s_awrr_igotyou", "s_awrr_ifoundyou", "s_awrr_gotyou"];
    ...
    level.battlechatter.eventstrings["RU"]["lost_target_water"] = ["s_lkpw_theywentunderwater", "s_lkpw_theyreinthewater", "s_lkpw_theyjumpedinthewater", "s_lkpw_targetsinthewater"];
    level.battlechatter.eventstrings["RU"]["ask_target_location"] = ["s_lklr_gotavisual", "s_lklr_youseeem", "s_lklr_whatstheirlocation", "s_lklr_wherearethey", "s_lklr_wherethefuckarethey", "s_lklr_getmealocation", "c_tast_wheresheat", "c_tast_doesanyonehaveavisua", "c_tast_wherethefuckretheyat"];
    ...
    level.battlechatter.eventstrings["RU"]["cmb_hunt_confirmation"] = ["s_cmhr_rog", "s_cmhr_rogerthat", "s_cmhr_copy", "s_cmhr_copythat", "s_cmhr_negative"];
    ...
    level.battlechatter.eventstrings["RU"]["hunt"] = ["s_aqhb_wherethefuckareyou", "s_aqhb_motherfuckers", "s_aqhb_yourehidingiknowyour", "s_aqhb_youreheresomewherear", "s_aqhb_comeonout", "s_aqhb_gonnafindyou", "s_aqhb_wheredyoufuckinggo"];
    ...
    level.battlechatter.eventstrings["RU"]["hunt_reactions"] = ["s_ahnr_surprisedinttenserea", "s_ahnr_surprisedinttenserea_01", "s_ahnr_surprisedinttenserea_02"];
    level.battlechatter.eventstrings["RU"]["cmb_reactions"] = ["s_aqcr_surprisedinttenserea", "s_aqcr_surprisedinttenserea_01", "s_aqcr_surprisedinttenserea_02"];
    level.battlechatter.eventstrings["RU"]["whizby_resp"] = ["s_aqal_itcamefromoverthere", "s_aqal_itcamefromthatway", "s_aqal_itwasfromoverthere", "s_unrc_itcamefromoverthere", "s_unrc_camefromthatway", "s_unrc_thatwaythatway", "s_unrc_itwasoverthatway", "s_unrc_itcamefromthatdirect"];
    ...
    level.battlechatter.eventstrings["RU_L"]["reply_hear_you"] = ["s_idrp_rogergoahead", "s_idrp_go", "s_idrp_goahead", "s_idrp_thisisactualgoahead", "s_idrp_thisisactual", "s_idrp_goforactual"];
    ...
    level.battlechatter.eventstrings["RU_L"]["wait_target_surface"] = ["s_lkwr_waitforemtocomeupthe", "s_lkwr_whentheycomeupforair", "s_lkwr_whentheycomeupyourec", "s_lkwr_whentheygoforairyout", "s_lkwr_lightemupwhentheygof", "s_lkwr_waitforemtosurfaceth"];
    ...
    level.battlechatter.eventstrings["RU_L"]["hunt_report_alert"] = ["s_aqhr_allshadowswehaveenem", "s_aqhr_allshadowsenemyisins", "s_aqhr_allshadowsperimeteri", "s_aqlo_fanoutsecuretheperim", "s_aqlo_setcontainmentfindem", "s_aqlo_spreadoutsearchthear"];
    ...
    level.battlechatter.eventstrings["RU_L"]["order_stay_aware"] = ["s_aqho_dontlettheseguysgett", "s_aqho_theseguysaretf141don", "s_aqho_keepyourguardupwitht"];
    level.battlechatter.eventstrings["RU_L"]["ask_how_copy"] = ["s_idcm_howcopy", "s_idcm_doyoucopy", "s_idcm_commscheck"];
    level.battlechatter.eventstrings["RU_L"]["inv_no_response"] = ["s_iinr_hesradiosilent", "s_iinr_icantreachhim", "s_iinr_hesnotoncomms", "s_iinr_hesnotresponding"];
    ...
    level.battlechatter.eventstrings["RU_L"]["report_target_location"] = ["s_chrp_doesanyonehaveavisua", "s_chrp_talktomeguyswhatsthe", "s_chrp_cmonshadowsineedaloc", "s_chrp_allshadowsdoyouhavea"];
    ...
    level.battlechatter.eventstrings["KON"]["left"] = ["c_posn_leftside", "c_posn_left", "c_posn_checkleft"];
    level.battlechatter.eventstrings["KON"]["right"] = ["c_posn_rightside", "c_posn_right", "c_posn_checkright"];
    level.battlechatter.eventstrings["KON"]["ahead"] = ["c_posn_straightahead", "c_posn_rightinfrontofus", "c_posn_aheadofus", "c_posn_front", "c_posn_deadahead"];
    level.battlechatter.eventstrings["KON"]["behind"] = ["c_posn_behindus", "c_posn_onoursix", "c_posn_checkrear", "c_posn_rear"];
    level.battlechatter.eventstrings["KON"]["high"] = ["c_posn_uptop", "c_posn_checkhigh"];
    level.battlechatter.eventstrings["KON"]["use_flash"] = ["c_ator_useaflashbang", "c_ator_throwaflashbang", "c_ator_tossaflashbang"];
    level.battlechatter.eventstrings["KON"]["use_grenade"] = ["c_ator_useagrenade", "c_ator_throwafrag", "c_ator_tossafrag"];
    level.battlechatter.eventstrings["KON"]["use_molotov"] = ["c_ator_throwamolotov", "c_ator_useamolotov", "c_ator_tossamolotov"];
    level.battlechatter.eventstrings["KON"]["use_rpg"] = ["c_ator_hitemwitharocket", "c_ator_usearocket", "c_ator_rocket"];
    level.battlechatter.eventstrings["KON"]["order_attack"] = ["c_ator_engage", "c_ator_giveemsome", "c_ator_dumpem", "c_ator_gohot", "c_ator_fuckinshootem", "c_ator_startsmokinem"];
    level.battlechatter.eventstrings["KON"]["attacking"] = ["c_atac_goinghot", "c_atac_engaging", "c_atac_openinup"];
    level.battlechatter.eventstrings["KON"]["using_rpg"] = ["c_atac_rocket", "c_atac_rocket_01"];
    level.battlechatter.eventstrings["KON"]["using_smoke"] = ["c_atac_throwingsmoke", "c_atac_smokeout"];
    level.battlechatter.eventstrings["KON"]["using_flash"] = ["c_atac_flashbangout", "c_atac_throwingflashbang"];
    level.battlechatter.eventstrings["KON"]["using_grenade"] = ["c_atac_fragout", "c_atac_throwingafrag"];
    level.battlechatter.eventstrings["KON"]["using_molotov"] = ["c_atac_throwingmolotov", "c_atac_molotovout"];
    level.battlechatter.eventstrings["KON"]["movingup"] = ["c_tcac_impushingup", "c_tcac_pushingforward", "c_tcac_movingup", "c_tcac_pushinup"];
    level.battlechatter.eventstrings["KON"]["getting_cover"] = ["c_tcac_covercover", "c_tcac_takingcover", "c_tcac_movingtocover"];
    level.battlechatter.eventstrings["KON"]["reloading"] = ["c_tcac_reloading", "c_tcac_changingmags"];
    level.battlechatter.eventstrings["KON"]["moving"] = ["c_tcac_moving", "c_tcac_immoving", "c_tcac_heyimmoving"];
    level.battlechatter.eventstrings["KON"]["enemy_using_flash"] = ["c_eata_flashincoming", "c_eata_flashgrenade", "c_eata_flash", "c_eata_flashgrenade_01"];
    level.battlechatter.eventstrings["KON"]["enemy_using_grenade"] = ["c_eata_grenade", "c_eata_grenadewatchout", "c_eata_grenadeincoming"];
    level.battlechatter.eventstrings["KON"]["enemy_using_molotov"] = ["c_eata_molotov", "c_eata_molotovincoming", "c_eata_molotov_01"];
    level.battlechatter.eventstrings["KON"]["enemy_using_rpg"] = ["c_eata_rpg", "c_eata_rpgincoming", "c_eata_rpggetdown"];
    level.battlechatter.eventstrings["KON"]["enemy_attacking"] = ["c_eata_takingfire", "c_eata_contact", "c_eata_shadowsincontact", "s_unrc_takingcontact", "s_unrc_contactcontact", "s_unrc_takingindirectfire"];
    level.battlechatter.eventstrings["KON"]["enemy_moving_up"] = ["c_etca_onespushing", "c_etca_gotonemovingup", "c_etca_onesmovingup"];
    level.battlechatter.eventstrings["KON"]["enemy_getting_cover"] = ["c_etca_novisualontarget", "c_etca_gotonetakingcover", "c_etca_onesmovingtocover", "c_etca_novisualontarget_01"];
    level.battlechatter.eventstrings["KON"]["enemy_reloading"] = ["c_etca_onesreloading", "c_etca_onesreloading_01", "c_etca_oneschangingmags"];
    level.battlechatter.eventstrings["KON"]["enemy_flanking_left"] = ["c_etca_onesmovingleft", "c_etca_leftflank", "c_etca_guardleft", "c_posn_theyreflankingleft"];
    level.battlechatter.eventstrings["KON"]["enemy_flanking_right"] = ["c_etca_guardright", "c_etca_rightflank", "c_etca_onesmovingright", "c_posn_theyreflankingright"];
    level.battlechatter.eventstrings["KON"]["enemy_moving"] = ["c_etca_onesmovin", "c_etca_thatonesmoving", "c_etca_gotonemoving"];
    level.battlechatter.eventstrings["KON"]["target_getting_cover"] = ["c_ttca_theyremovingtocover", "c_ttca_hesgoingforcover", "c_ttca_hestakingcover"];
    level.battlechatter.eventstrings["KON"]["target_reloading"] = ["c_ttca_theyrereloading", "c_ttca_hesout", "c_ttca_hesreloading"];
    level.battlechatter.eventstrings["KON"]["target_moving"] = ["c_ttca_theyremoving", "c_ttca_hesmoving", "c_ttca_hesrunning"];
    level.battlechatter.eventstrings["KON"]["target_moving_up"] = ["c_ttca_theyremovingup", "c_ttca_hescominguponyou", "c_ttca_hesmovingup"];
    level.battlechatter.eventstrings["KON"]["target_near_patrol"] = ["c_ttca_theyreswimmingnearth", "c_ttca_theyreswimmingyourwa", "c_ttca_interceptthem"];
    level.battlechatter.eventstrings["KON"]["enemy_soldier"] = ["c_enmy_visualonatarget", "c_enmy_targetspotted", "c_enmy_gotatarget"];
    level.battlechatter.eventstrings["KON"]["enemy_soldiers"] = ["c_enmy_targets", "c_enmy_targets_01", "c_enmy_wegotbadguys"];
    level.battlechatter.eventstrings["KON"]["neg_target"] = ["c_aqen_idontseeem", "c_aqen_novisual", "c_aqen_cantseehim", "c_aqen_wherethefuckishe"];
    level.battlechatter.eventstrings["KON"]["aquired_target"] = ["c_aqen_gotavisual", "c_aqen_targetacquired", "c_aqen_iseehim", "c_aqen_thereheis"];
    level.battlechatter.eventstrings["KON"]["enemy_exposed"] = ["c_exco_onesintheopen", "c_exco_onesintheopen_01", "c_exco_gotoneintheopen"];
    level.battlechatter.eventstrings["KON"]["target_exposed"] = ["c_exco_theyreintheopen", "c_exco_hesintheopen", "c_exco_theyreintheopen_01"];
    level.battlechatter.eventstrings["KON"]["youre_exposed"] = ["c_exco_findcover", "c_exco_gettocoversoldier", "c_exco_youreexposedsoldierm"];
    level.battlechatter.eventstrings["KON"]["target_covered"] = ["c_exco_hesbehindcover", "c_exco_hesgotcover", "c_exco_hestakingcover"];
    level.battlechatter.eventstrings["KON"]["killfirm_bigrig"] = ["c_firm_theyresemitrucksdown", "c_firm_destroyedtheirsemitr", "c_firm_enemysemitrucksdown"];
    level.battlechatter.eventstrings["KON"]["killfirm_enemy"] = ["c_firm_gotone", "c_firm_ekia", "c_firm_targetdown"];
    level.battlechatter.eventstrings["KON"]["killfirm_target"] = ["c_firm_hesdone", "c_firm_hesdown", "c_firm_dumpedhim"];
    level.battlechatter.eventstrings["KON"]["killfirm_truck"] = ["c_firm_truckdestroyed", "c_firm_theirtrucksdown", "c_firm_enemytrucksdown"];
    level.battlechatter.eventstrings["KON"]["killfirm_helo"] = ["c_firm_helodown", "c_firm_helodestroyed", "c_firm_theirhelosdown"];
    level.battlechatter.eventstrings["KON"]["casualties"] = ["c_casu_weretakingheavies", "c_casu_weregettingourassesk", "c_casu_werelosingshooters", "c_casu_weretakingcasualties"];
    level.battlechatter.eventstrings["KON"]["copy"] = ["c_conf_copy", "c_conf_check", "c_conf_copythat", "c_conf_goodcopy", "c_conf_solidcopy", "c_aqcp_copy", "c_aqcp_solidcopy"];
    level.battlechatter.eventstrings["KON"]["praise"] = ["c_pras_hellyeah", "c_pras_niceone", "c_pras_fuckina", "c_pras_yeah", "c_pras_getyousome"];
    level.battlechatter.eventstrings["KON"]["low_ammo"] = ["c_stat_ineedamag", "c_stat_lowonammo", "c_stat_lastmag", "c_stat_needammo", "c_stat_imwinchester"];
    level.battlechatter.eventstrings["KON"]["wounded"] = ["c_stat_imhit", "c_stat_imshot", "c_stat_medic"];
    level.battlechatter.eventstrings["KON"]["ask_wounded"] = ["c_stat_yougood", "c_stat_youwounded", "c_stat_youhit"];
    level.battlechatter.eventstrings["KON"]["ask_low_ammo"] = ["c_stat_howsyourammo", "c_stat_checkyourammo", "c_stat_whatsyourroundcount"];
    level.battlechatter.eventstrings["KON"]["ask_status"] = ["c_stat_youokay", "c_stat_whatsyourstatus", "c_stat_status"];
    level.battlechatter.eventstrings["KON"]["reply_wounded"] = ["c_stat_itookaround", "c_stat_imgood", "c_stat_yeahroundhittheplate"];
    level.battlechatter.eventstrings["KON"]["reply_low_ammo"] = ["c_stat_runninglow", "c_stat_imlow", "c_stat_almostout"];
    level.battlechatter.eventstrings["KON"]["reply_okay"] = ["c_stat_imsolid", "c_stat_goodtogo", "c_stat_goodhere"];
    level.battlechatter.eventstrings["KON"]["wounded_enemy"] = ["c_aens_igothim", "c_aens_clippedhim", "c_aens_hitone"];
    level.battlechatter.eventstrings["KON"]["ask_target_wounded"] = ["c_tast_yougethim", "c_tast_youhithim", "c_tast_youdrophim"];
    level.battlechatter.eventstrings["KON"]["target_unhurt"] = ["c_tast_hegotaway", "c_tast_heranoff", "c_tast_hesfallingback"];
    level.battlechatter.eventstrings["KON"]["wounded_target"] = ["c_atas_woundedhim", "c_atas_ihithim", "c_atas_itaggedem", "c_tast_heshit", "c_tast_heswounded", "c_tast_hesbleeding"];
    level.battlechatter.eventstrings["KON"]["enemy_shotgun"] = ["c_enws_shotgunwatchout", "c_enws_onesgotashotgun", "c_enws_shotgun"];
    level.battlechatter.eventstrings["KON"]["enemy_sniper"] = ["c_enws_thatsasniper", "c_enws_sniper", "c_enws_enemysniper"];
    level.battlechatter.eventstrings["KON"]["enemy_lmg"] = ["c_enws_enemymachinegunner", "c_enws_lmg", "c_enws_machinegun"];
    level.battlechatter.eventstrings["KON"]["target_lmg"] = ["c_taws_theyreusingamachineg", "c_taws_hesgotamachinegun", "c_taws_machinegunner"];
    level.battlechatter.eventstrings["KON"]["target_shotgun"] = ["c_taws_theyvegotashotgun", "c_taws_hesgotashotgun"];
    level.battlechatter.eventstrings["KON"]["target_sniper"] = ["c_taws_hessnipingus", "c_taws_hesasniper"];
    level.battlechatter.eventstrings["KON"]["checkfire"] = ["c_chck_checkyourfire", "c_chck_watchyourfire", "c_chck_blueblue", "c_chck_checkfirecheckfire"];
    level.battlechatter.eventstrings["KON"]["move"] = ["c_tcor_move", "c_tcor_igotyoumove", "c_tcor_startmoving", "c_tcor_go"];
    level.battlechatter.eventstrings["KON"]["moveup"] = ["c_tcor_moveup", "c_tcor_moveupgo", "c_tcor_pushup", "c_tcor_getupthere", "c_tcor_pushforward"];
    level.battlechatter.eventstrings["KON"]["coverme"] = ["c_tcor_coverme", "c_tcor_ineedcoverfire", "c_tcor_coverme_01"];
    level.battlechatter.eventstrings["KON"]["casualty"] = ["c_canm_wegotacasualty", "c_canm_shadowdown", "c_canm_wehaveashadowdown", "c_canm_wevegotwounded", "c_canm_mandown"];
    level.battlechatter.eventstrings["KON"]["suppressing"] = ["c_aqsp_suppressingfire", "c_aqsp_imcoveringyou", "c_aqsp_coveringfire"];
    level.battlechatter.eventstrings["KON"]["enemy_covered"] = ["c_exco_onesbehindcover", "c_exco_oneofthemisbehindcov", "c_exco_gotoneofthembehindco"];
    level.battlechatter.eventstrings["KON"]["use_smoke"] = ["c_atac_throwasmokegrenade", "c_atac_useasmokegrenade", "c_atac_tossasmokegrenade"];
    level.battlechatter.eventstrings["KON"]["enemy_grenade_close"] = ["c_glna_fuckpleaseno", "c_glna_fuck", "c_glna_noplease"];
    level.battlechatter.eventstrings["KON"]["team_enemy_grenade_close"] = ["c_glna_fuckweredead", "c_glna_werefuckingdead", "c_glna_fucknoweredead"];
    level.battlechatter.eventstrings["KON"]["hurt_by_molotov"] = ["c_mtvd_fuckthatburns", "c_mtvd_ahfuckthatshot", "c_mtvd_ahthatsfuckinghot"];
    level.battlechatter.eventstrings["KON"]["reply_keepshooting"] = ["c_stat_keepshootinguntilthe", "c_stat_thenkeepshootingthem", "c_stat_useeverylastbulletto"];
    level.battlechatter.eventstrings["KON"]["enemy_unhurt"] = ["c_tast_ijustmissedone", "c_tast_justmissedone", "c_tast_barelymissedone"];
    level.battlechatter.eventstrings["KON"]["lost_enemy_duo"] = ["c_tast_welostem", "c_tast_welostthem", "c_tast_welosttrackofem"];
    level.battlechatter.eventstrings["KON"]["lost_enemy_solo"] = ["c_tast_ilosttrackofthem", "c_tast_fuckilostem", "c_tast_fuckilostthem"];
    level.battlechatter.eventstrings["KON"]["repositioning"] = ["c_posn_mycoversblownimrepos", "c_posn_imrepositioning", "c_posn_gottoreposition"];
    level.battlechatter.eventstrings["KON"]["reply_hear_you"] = ["s_idrp_go", "s_idrp_goahead", "s_idrp_imhere", "s_idrp_rogergoahead"];
    level.battlechatter.eventstrings["KON"]["resp_patrol"] = ["s_idrp_thisisthresher", "s_ahrp_thisisthresher"];
    level.battlechatter.eventstrings["KON"]["muttered_confusion"] = ["s_idin_whatthefuck", "s_idin_whatthehell", "s_idin_whatthefuckwasthat", "s_idin_whatthehellwasthat"];
    level.battlechatter.eventstrings["KON"]["investigating"] = ["s_idin_illseewhatsup", "s_idin_illcheckitout", "s_idin_illgocheck"];
    level.battlechatter.eventstrings["KON"]["react_found_corpse"] = ["s_alrc_wegotamandown", "s_alrc_wegotacasualty", "s_alrc_gotashadowdown", "s_alrc_wegotakia"];
    level.battlechatter.eventstrings["KON"]["ask_corpse_name"] = ["s_alrc_whoisit", "s_alrc_who", "s_alrc_whodidwelose"];
    level.battlechatter.eventstrings["KON"]["need_backup"] = ["s_alrc_sendshadows", "s_alrc_sendreinforcements", "s_albc_sendreinforcements"];
    level.battlechatter.eventstrings["KON"]["hunt_copy"] = ["s_hnrs_copy", "s_hnrs_rogerthat", "s_hnrs_copythat", "s_hnrs_rog"];
    level.battlechatter.eventstrings["KON"]["hunt_neg"] = ["s_hnrs_negative", "s_hnrs_no", "s_hnrs_nosir"];
    level.battlechatter.eventstrings["KON"]["react_whizby"] = ["s_unrc_imtakingfire", "s_unrc_thatsclose", "s_unrc_someonesshootinatme", "s_unrc_takineffectivefire", "s_unrc_imgettingshotat"];
    level.battlechatter.eventstrings["KON"]["over_there"] = ["s_hnfn_overthere", "s_hnfn_theyreoverthere", "s_hnfn_rightthere"];
    level.battlechatter.eventstrings["KON"]["surprise"] = ["s_hurc_ohfuck", "s_hurc_ohshit"];
    level.battlechatter.eventstrings["KON"]["report_reply_idle"] = ["s_idrr_gotnothing", "s_idrr_secure", "s_idrr_clearhere", "s_idrr_allclear", "s_idrr_goodhere", "s_idrr_allquiet", "s_idrr_areassecure"];
    level.battlechatter.eventstrings["KON"]["react_first"] = ["s_idrc_huh", "s_idrc_wha", "s_idrc_hm", "s_idrc_eh", "s_idrc_hrm", "s_idrc_hmph", "s_idin_hmm", "s_idin_hmphokay", "s_idin_huhwhatwasthat", "s_idin_hrm", "s_idin_huhthehell"];
    level.battlechatter.eventstrings["KON"]["react_second"] = ["s_idrc_whatthefuckisthat", "s_idrc_whatthehellisthat", "s_idrc_thefuckisgoingon"];
    level.battlechatter.eventstrings["KON"]["react_multi"] = ["s_idrm_whosoutthere", "s_idrm_okaywhatthefuck", "s_idrm_whatisthisshit"];
    level.battlechatter.eventstrings["KON"]["react_interrupt"] = ["s_idli_missedyourlastover", "s_idli_sayagainover", "s_idli_yougood"];
    level.battlechatter.eventstrings["KON"]["react_noreply"] = ["s_idli_youreceivingme", "s_idli_youcopy", "s_idli_doyoureadme"];
    level.battlechatter.eventstrings["KON"]["order_investigate"] = ["s_idio_gocheckit", "s_idio_checkitout", "s_idio_gocheckthatout"];
    level.battlechatter.eventstrings["KON"]["confirming"] = ["s_idlc_roger", "s_idlc_copy", "s_idlc_copythat"];
    level.battlechatter.eventstrings["KON"]["idle_good_hunting"] = ["s_idls_stayfrosty", "s_idls_keepittight", "s_idls_staysharp"];
    level.battlechatter.eventstrings["KON"]["inv_ask_first"] = ["s_idia_whatwasit", "s_idia_whatdyoufind", "s_idia_yougotanything", "s_idia_seeanything", "s_idia_findanything", "s_alia_seeanything", "s_alia_yougotsomething", "s_alia_anything", "s_alia_gotanything"];
    level.battlechatter.eventstrings["KON"]["inv_ask_second"] = ["s_idia_whatwasitthistime", "s_idia_anythingthistime", "s_idia_stillnothing", "s_idia_stilldidntfindanythi", "s_idia_findanythingthistime"];
    level.battlechatter.eventstrings["KON"]["resp_window_first"] = ["s_idir_awindowwasleftopenwe", "s_idir_foundanopenwindowche", "s_idir_someoneleftawindowop"];
    level.battlechatter.eventstrings["KON"]["resp_water_first"] = ["s_idir_checkedthewaterthere", "s_idir_sweptthewaterdidntfi", "s_idir_checkednearthewaterw", "s_idir_nothinginthewaterwer"];
    level.battlechatter.eventstrings["KON"]["resp_sound_first"] = ["s_idir_nothingnobody", "s_idir_negativewereclear", "s_idir_nahweresecure", "s_idir_nothingnovisual"];
    level.battlechatter.eventstrings["KON"]["resp_window_multi"] = ["s_idir_sawanotheropenwindow", "s_idir_anotheropenwindowwer", "s_idir_therewasanotheropenw"];
    level.battlechatter.eventstrings["KON"]["order_check_together"] = ["s_idit_letsgocheckitout", "s_idit_letscheckitwatchmyba", "s_idit_letsgocheckcoverme"];
    level.battlechatter.eventstrings["KON"]["covering_investigate"] = ["s_idit_covering", "s_idit_imwithyou", "s_idit_gotyoucovered"];
    level.battlechatter.eventstrings["KON"]["team_return_idle"] = ["s_idid_okayletsheadback", "s_idid_weregoodhereletsgetb", "s_idid_backtoourpostsletsgo"];
    level.battlechatter.eventstrings["KON"]["ask_heard_that"] = ["s_alre_youhearthat", "s_alre_youheardthatright", "s_alre_youheardthattoo"];
    level.battlechatter.eventstrings["KON"]["ask_saw_that"] = ["s_alre_didyouseethat", "s_alre_yougeteyesonthat", "s_alre_youcatchthat"];
    level.battlechatter.eventstrings["KON"]["have_something"] = ["s_alre_imighthavesomething", "s_alre_ithinkihavesomething", "s_alre_igotapossiblethreat"];
    level.battlechatter.eventstrings["KON"]["heard_something"] = ["s_alre_iheardsomething", "s_alre_heardsomethingnearme", "s_alre_iheardanoise"];
    level.battlechatter.eventstrings["KON"]["saw_something"] = ["s_alre_isawsomething", "s_alre_thoughtisawsomething", "s_alre_ithinksomeonesnearme"];
    level.battlechatter.eventstrings["KON"]["see_something"] = ["s_alre_iseesomething", "s_alre_ithinkiseesomething", "s_alre_iseesomethingnearme"];
    level.battlechatter.eventstrings["KON"]["team_have_something"] = ["s_alre_wemighthavesomething", "s_alre_ithinkwegotsomething", "s_alre_wegotapossiblethreat"];
    level.battlechatter.eventstrings["KON"]["team_heard_something"] = ["s_alre_weheardsomething", "s_alre_weheardsomethingclos", "s_alre_weheardanoise"];
    level.battlechatter.eventstrings["KON"]["team_saw_something"] = ["s_alre_wesawsomething", "s_alre_wesawsomethingnearus", "s_alre_wesawsomethingnearou"];
    level.battlechatter.eventstrings["KON"]["team_see_something"] = ["s_alre_weseesomething"];
    level.battlechatter.eventstrings["KON"]["bottle_whizby"] = ["s_alre_whothefuckthrewthat", "s_alre_whothrewthat", "s_alre_youmissed"];
    level.battlechatter.eventstrings["KON"]["react_resp"] = ["s_alrr_yeah", "s_alrr_afirm", "s_alrr_yeahrogerthat"];
    level.battlechatter.eventstrings["KON"]["alert_investigating"] = ["s_alin_imgonnareccethearea", "s_alin_imcheckingitout", "s_alin_imgonnacheckitout"];
    level.battlechatter.eventstrings["KON"]["team_alert_investigating"] = ["s_alin_wellcheckthearea", "s_alin_werecheckingitout", "s_alin_weregonnacheckitout"];
    level.battlechatter.eventstrings["KON"]["alert_confirming"] = ["s_alcf_roger", "s_alcf_copy", "s_alcf_copythat"];
    level.battlechatter.eventstrings["KON"]["alert_patrol"] = ["s_alip_illpullsecurityherea", "s_alip_illsecuretheareakeep", "s_alip_headbackillpullsecur", "s_alip_ivegotthisareagetbac"];
    level.battlechatter.eventstrings["KON"]["report_reply_search"] = ["s_alir_negativenothingyet", "s_alir_nothingsofar", "s_alir_nothing", "s_alir_goodsofar", "s_alir_igotnothingyet", "s_alir_sofarsogood", "s_alir_notseeinganything", "s_alir_negativegotnothing", "s_alir_nothingfornow", "s_alir_itsclearsofar", "s_alir_weregoodfornow"];
    level.battlechatter.eventstrings["KON"]["warn_report"] = ["s_wrnr_hadanunknowninsideth", "s_wrnr_hadanunknowninsideth_01", "s_wrnr_anunknownwasintheare"];
    level.battlechatter.eventstrings["KON"]["warn_spotted"] = ["s_wrnf_heygetthefuckback", "s_wrnf_heyyougetthefuckoutt", "s_wrnf_getbackgetbackrightn"];
    level.battlechatter.eventstrings["KON"]["warn_conv"] = ["s_wrni_scaredthehelloutofem", "s_wrni_probablyshittingthem", "s_wrni_whatthehellweretheyd", "s_wrni_whatthefuckweretheyt"];
    level.battlechatter.eventstrings["KON"]["warn_conv_reply"] = ["s_wrir_almostgotthemselvesk", "s_wrir_iwasreadytodumpthatm", "s_wrir_luckytheydidntcatcha"];
    level.battlechatter.eventstrings["KON"]["warn_attack"] = ["s_wrnk_heyiwarnedyou", "s_wrnk_youwerewarned", "s_wrnk_heyyoujustfuckedup"];
    level.battlechatter.eventstrings["KON"]["ask_enemy_loc"] = ["s_unwl_wheredthatcomefrom", "s_unwl_whereditcomefrom", "s_unwl_wherethefuckarethey"];
    level.battlechatter.eventstrings["KON"]["enemy_loc_resp"] = ["s_unrs_idontknowbutthatwasa", "s_unrs_ididntseeem", "s_unrs_idontfuckingknow", "s_unrs_novisualdidntseeem"];
    level.battlechatter.eventstrings["KON"]["ask_enemy_count"] = ["s_unwc_howmanyarethere", "s_unwc_howmanyofem", "s_unwc_howmany"];
    level.battlechatter.eventstrings["KON"]["unaware_backup"] = ["s_unwb_ineedreinforcements", "s_unwb_getmoreshadowshereno", "s_unwb_sendreinforcementsmy"];
    level.battlechatter.eventstrings["KON"]["react_first_combat"] = ["s_awrr_contact", "s_awrr_troopsincontact", "s_awrr_enemymyposition", "s_awrr_enemy", "s_awrr_enemyshere", "s_awrr_gotenemyhere", "s_awrr_enemysclose"];
    level.battlechatter.eventstrings["KON"]["report_target_water"] = ["s_awrr_targetsinthewater", "s_awrr_gotavisualtheyreinth", "s_awrr_iseeeminthewater", "s_awrr_inthewaterinthewater", "s_awrr_enemyinthewater", "s_awrr_theyremovinginthewat", "s_awrr_heytheyreinthewater"];
    level.battlechatter.eventstrings["KON"]["at_my_location"] = ["s_awrr_righthere", "s_awrr_onmeonme", "s_awrr_myposition", "s_awrr_righthererighthere", "s_awrr_onme", "s_awrr_theyrerighthere", "s_awrr_overhere", "s_awrr_theyreoverhere"];
    level.battlechatter.eventstrings["KON"]["found_target"] = ["s_awrr_gotem", "s_awrr_iseeem", "s_awrr_foundem", "s_awrr_hey", "s_awrr_visual", "s_awrr_targetspotted"];
    level.battlechatter.eventstrings["KON"]["found_you"] = ["s_awrr_motherfucker", "s_awrr_foundyou", "s_awrr_igotyou", "s_awrr_ifoundyou", "s_awrr_gotyou"];
    level.battlechatter.eventstrings["KON"]["order_check_last"] = ["s_advo_moveonemgo", "s_advo_forceuponemgo", "s_advo_pushtheirpositiongo"];
    level.battlechatter.eventstrings["KON"]["lost_target"] = ["s_lkpl_frustratedgrunttheyr", "s_lkpl_frustratedgrunttheyr_01", "s_lkpl_frustratedgruntnovis", "s_lkpl_frustratedgrunttheyg"];
    level.battlechatter.eventstrings["KON"]["lost_target_water"] = ["s_lkpw_theywentunderwater", "s_lkpw_theyreinthewater", "s_lkpw_theyjumpedinthewater", "s_lkpw_targetsinthewater"];
    level.battlechatter.eventstrings["KON"]["ask_target_location"] = ["s_lklr_gotavisual", "s_lklr_youseeem", "s_lklr_whatstheirlocation", "s_lklr_wherearethey", "s_lklr_wherethefuckarethey", "s_lklr_getmealocation", "c_tast_wheresheat", "c_tast_doesanyonehaveavisua", "c_tast_wherethefuckretheyat"];
    level.battlechatter.eventstrings["KON"]["wait_target_surface"] = ["s_lkwr_whentheycomeupsmokee", "s_lkwr_theypopupyoushootem", "s_lkwr_whentheysurfaceopenu", "s_lkwr_theygoforairyoudrope", "s_lkwr_waittiltheysurfaceth", "s_lkwr_theysurfaceyoulighte"];
    level.battlechatter.eventstrings["KON"]["water_use_grenade"] = ["s_lkwf_dontwasteammousegren", "s_lkwf_throwagrenadeinthere", "s_lkwf_getagrenadeinthewate"];
    level.battlechatter.eventstrings["KON"]["target_still_close"] = ["s_lkpc_theywerehere", "s_lkpc_theywererighthere", "s_lkpc_theycantbefar", "s_lkpc_theyreclose", "s_lkpc_theyrehiding", "s_lkpc_theygottabeclose", "s_lkpc_theydidntmakeitfar", "s_lkpc_theyrestillheresomew"];
    level.battlechatter.eventstrings["KON"]["order_clear_building"] = ["s_cmho_clearthebuilding", "s_cmho_securethebuilding", "s_cmho_getthisbuildingsecur"];
    level.battlechatter.eventstrings["KON"]["order_search_water"] = ["s_cmho_searchthewater", "s_cmho_checkthewater", "s_cmho_targetwasinthewater", "s_cmho_geteyesonthewater", "s_cmho_watchthewater"];
    level.battlechatter.eventstrings["KON"]["clearing_east"] = ["s_cmhs_checkingeast", "s_cmhs_movingeast", "s_cmhs_securingeast"];
    level.battlechatter.eventstrings["KON"]["clearing_north"] = ["s_cmhs_checkingnorth", "s_cmhs_movingnorth", "s_cmhs_securingnorth"];
    level.battlechatter.eventstrings["KON"]["clearing_south"] = ["s_cmhs_checkingsouth", "s_cmhs_movingsouth", "s_cmhs_securingsouth"];
    level.battlechatter.eventstrings["KON"]["clearing_west"] = ["s_cmhs_checkingwest", "s_cmhs_movingwest", "s_cmhs_securingwest"];
    level.battlechatter.eventstrings["KON"]["searching_here"] = ["s_cmhc_checkingthisarea", "s_cmhc_securingthisarea", "s_cmhc_searchingthisarea"];
    level.battlechatter.eventstrings["KON"]["cmb_hunt_confirmation"] = ["s_cmhr_rog", "s_cmhr_rogerthat", "s_cmhr_copy", "s_cmhr_copythat", "s_cmhr_negative"];
    level.battlechatter.eventstrings["KON"]["good_hunting"] = ["s_hnts_staysharp", "s_hnts_stayfrosty", "s_hnts_keepittight"];
    level.battlechatter.eventstrings["KON"]["moving_there"] = ["s_hntm_movingnow", "s_hntm_movingtherenow", "s_hntm_moving"];
    level.battlechatter.eventstrings["KON"]["moving_alone"] = ["s_hntm_iminbound", "s_hntm_immoving", "s_hntm_immovingnow"];
    level.battlechatter.eventstrings["KON"]["moving_following"] = ["s_hntm_gotyoursix", "s_hntm_imwithyou", "s_hntm_gotyourback"];
    level.battlechatter.eventstrings["KON"]["moving_together"] = ["s_hntm_weremoving", "s_hntm_weremovingtherenow", "s_hntm_wereinboundnow"];
    level.battlechatter.eventstrings["KON"]["order_follow_me"] = ["s_hntm_illleadletsgo", "s_hntm_onme", "s_hntm_illtakepointletsmove"];
    level.battlechatter.eventstrings["KON"]["order_take_point"] = ["s_hntm_takepointmoveout", "s_hntm_youtakepointletsgo", "s_hntm_onyouletsmove", "s_aqhr_takeitoutigotyourbac", "s_aqhr_takepointillcover", "s_aqhr_takepointletsroll"];
    level.battlechatter.eventstrings["KON"]["ask_found_anything"] = ["s_aqhr_gotanything", "s_aqhr_yougotsomething", "s_aqhr_findanything", "s_aqhr_gotavisual", "s_aqhr_anyvisual", "s_aqhr_whatdoyougot"];
    level.battlechatter.eventstrings["KON"]["reply_found_nothing"] = ["s_aqhr_novisualonthetarget", "s_aqhr_nosignofem", "s_aqhr_novisual"];
    level.battlechatter.eventstrings["KON"]["report_searching"] = ["s_aqhr_stillsweeping", "s_aqhr_stillsearching", "s_aqhr_stilllookingforem"];
    level.battlechatter.eventstrings["KON"]["report_nothing"] = ["s_aqhr_nothingyet", "s_aqhr_nothingsofar", "s_aqhr_nothinghere"];
    level.battlechatter.eventstrings["KON"]["report_investigating"] = ["s_aqhr_immovingin", "s_aqhr_imcheckingitout", "s_aqhr_movingtosecure"];
    level.battlechatter.eventstrings["KON"]["area_clear"] = ["s_aqhr_thisareassecure", "s_aqhr_securehere", "s_aqhr_areasecure"];
    level.battlechatter.eventstrings["KON"]["order_dir"] = ["s_aqhs_securethatarea", "s_aqhs_clearthatarea", "s_aqhs_cordonthatarea", "s_aqhs_checkthatway"];
    level.battlechatter.eventstrings["KON"]["order_dir_resp"] = ["s_aqhs_illsecurethisarea", "s_aqhs_illtakethisway", "s_aqhs_illcheckoverhere", "s_aqhs_illsearchthisway"];
    level.battlechatter.eventstrings["KON"]["hunt_search_object"] = ["s_aqhs_itsclear", "s_aqhs_clear", "s_aqhs_secure", "s_aqhs_weresecure"];
    level.battlechatter.eventstrings["KON"]["hunt"] = ["s_aqhb_wherethefuckareyou", "s_aqhb_motherfuckers", "s_aqhb_yourehidingiknowyour", "s_aqhb_youreheresomewherear", "s_aqhb_comeonout", "s_aqhb_gonnafindyou", "s_aqhb_wheredyoufuckinggo"];
    level.battlechatter.eventstrings["KON"]["hunt_inv_order"] = ["s_ahii_checkit", "s_ahii_gocheckitout", "s_ahii_checkthatout", "s_ahii_gocheckthat"];
    level.battlechatter.eventstrings["KON"]["hunt_inv_event"] = ["s_ahir_ithinktheyrenearme", "s_ahir_imighthaveem", "s_ahir_ithinkigotem", "s_ahir_thinkifoundem"];
    level.battlechatter.eventstrings["KON"]["hunt_reactions"] = ["s_ahnr_surprisedinttenserea", "s_ahnr_surprisedinttenserea_01", "s_ahnr_surprisedinttenserea_02"];
    level.battlechatter.eventstrings["KON"]["cmb_reactions"] = ["s_aqcr_surprisedinttenserea", "s_aqcr_surprisedinttenserea_01", "s_aqcr_surprisedinttenserea_02"];
    level.battlechatter.eventstrings["KON"]["whizby_resp"] = ["s_aqal_itcamefromoverthere", "s_aqal_itcamefromthatway", "s_aqal_itwasfromoverthere", "s_unrc_itcamefromoverthere", "s_unrc_camefromthatway", "s_unrc_thatwaythatway", "s_unrc_itwasoverthatway", "s_unrc_itcamefromthatdirect"];
    level.battlechatter.eventstrings["KON"]["hunt_cornercheck"] = ["s_aqho_checkyourcorners", "s_aqho_checkcornersfindem", "s_aqho_checkthosecorners"];
    level.battlechatter.eventstrings["KON"]["hunt_order_hold"] = ["s_aqho_holdup", "s_aqho_standfast", "s_aqho_hold"];
    level.battlechatter.eventstrings["KON"]["alert_cornercheck"] = ["s_aqao_checkthecorners", "s_aqao_watchyourcorners", "s_aqao_checkthosecorners"];
    level.battlechatter.eventstrings["KON"]["alert_order_hold"] = ["s_aqao_holdup", "s_aqao_standfast", "s_aqao_hold"];
    level.battlechatter.eventstrings["KON"]["alert_search_orders"] = ["s_aqao_checkthatway", "s_aqao_gothatway", "s_aqao_checkoverthere", "s_aqao_searchoverthere", "s_aqao_searchthatway", "s_aqao_securethatarea"];
    level.battlechatter.eventstrings["KON"]["attack_orders"] = ["s_aato_thresher"];
    level.battlechatter.eventstrings["KON"]["lkp_patrol_search"] = ["s_alwp_justwatchthewater", "s_alwp_keepwatchingthatside", "s_alwp_justkeepyoureyesonth"];
    level.battlechatter.eventstrings["KON"]["hunt_report_alert"] = ["s_aqlo_cordonandsearchthear", "s_aqlo_fanoutsearchthearea", "s_aqlo_cordonofftheareafind"];
    level.battlechatter.eventstrings["KON"]["resp_glass_first"] = ["s_airs_brokenglasshere", "s_airs_itsbrokenglass", "s_airs_gotsomebrokenglass"];
    level.battlechatter.eventstrings["KON"]["resp_glass_multi"] = ["s_airs_moreglass", "s_airs_gotsomemoreglass", "s_airs_theresmorebrokenglas"];
    level.battlechatter.eventstrings["KON"]["flanked"] = ["s_awrr_ivegotthemflanked", "s_awrr_ivegotaflankonthem", "s_awrr_iveflankedthem"];
    level.battlechatter.eventstrings["KON_L"]["reply_hear_you"] = ["s_idrp_rogergoahead", "s_idrp_go", "s_idrp_goahead", "s_idrp_thisisactualgoahead", "s_idrp_thisisactual", "s_idrp_goforactual"];
    level.battlechatter.eventstrings["KON_L"]["react_interrupt"] = ["s_idli_missedyourlastover", "s_idli_sayagainover", "s_idli_yougood", "s_idli_sayagainyourlastover", "s_idli_commscheck", "s_idli_missedyourlastsayaga"];
    level.battlechatter.eventstrings["KON_L"]["react_noreply"] = ["s_idli_youreceivingme", "s_idli_youcopy", "s_idli_doyoureadme"];
    level.battlechatter.eventstrings["KON_L"]["confirming"] = ["s_idlc_roger", "s_idlc_copy", "s_idlc_copythat"];
    level.battlechatter.eventstrings["KON_L"]["signoff"] = ["s_idls_out", "s_idls_actualout", "s_idls_stayfrosty", "s_idls_out_01"];
    level.battlechatter.eventstrings["KON_L"]["inv_patrol"] = ["s_alip_continueyourpatrolst", "s_alip_getbacktoyourrouteke", "s_alip_returntoyourpatrolst"];
    level.battlechatter.eventstrings["KON_L"]["wait_target_surface"] = ["s_lkwr_waitforemtocomeupthe", "s_lkwr_whentheycomeupforair", "s_lkwr_whentheycomeupyourec", "s_lkwr_whentheygoforairyout", "s_lkwr_lightemupwhentheygof", "s_lkwr_waitforemtosurfaceth"];
    level.battlechatter.eventstrings["KON_L"]["order_find_target"] = ["s_cmho_weneedalocationonthe", "s_cmho_locatethetargetnow", "s_cmho_huntemdown"];
    level.battlechatter.eventstrings["KON_L"]["order_find_perimeter"] = ["s_cmho_getthisarealockeddow", "s_cmho_securetheperimeter", "s_cmho_iwantthisarealockedd"];
    level.battlechatter.eventstrings["KON_L"]["order_search_water"] = ["s_cmho_searchthewater", "s_cmho_checkthewater", "s_cmho_theywereinthewater", "s_cmho_keepyoureyesonthewat", "s_cmho_watchthewater"];
    level.battlechatter.eventstrings["KON_L"]["good_hunting"] = ["s_hnts_goodhunting", "s_hnts_staysharpoutthere", "s_hnts_out", "s_aisg_copystaysharp"];
    level.battlechatter.eventstrings["KON_L"]["ask_found_anything"] = ["s_aqhr_whatsyourstatus", "s_aqhr_whatveyougot", "s_aqhr_givemeasitrep", "s_aqhr_sitrep", "s_aqhr_youfindanything", "s_aqhr_tellmeyougotsomethin"];
    level.battlechatter.eventstrings["KON_L"]["hunt_report_alert"] = ["s_aqhr_allshadowswehaveenem", "s_aqhr_allshadowsenemyisins", "s_aqhr_allshadowsperimeteri", "s_aqlo_fanoutsecuretheperim", "s_aqlo_setcontainmentfindem", "s_aqlo_spreadoutsearchthear"];
    level.battlechatter.eventstrings["KON_L"]["order_continue_search"] = ["s_aqho_keeplookingforem", "s_aqho_keepsweeping", "s_aqho_keepsearchingthearea"];
    level.battlechatter.eventstrings["KON_L"]["order_stay_alert"] = ["s_aqho_staysharp", "s_aqho_keepittight", "s_aqho_eyesopen"];
    level.battlechatter.eventstrings["KON_L"]["order_stay_aware"] = ["s_aqho_dontlettheseguysgett", "s_aqho_theseguysaretf141don", "s_aqho_keepyourguardupwitht"];
    level.battlechatter.eventstrings["KON_L"]["ask_how_copy"] = ["s_idcm_howcopy", "s_idcm_doyoucopy", "s_idcm_commscheck"];
    level.battlechatter.eventstrings["KON_L"]["inv_no_response"] = ["s_iinr_hesradiosilent", "s_iinr_icantreachhim", "s_iinr_hesnotoncomms", "s_iinr_hesnotresponding"];
    level.battlechatter.eventstrings["KON_L"]["order_get_visual"] = ["s_iinr_seeifyoucanfindoutwh", "s_iinr_checkitoutmakesureno", "s_iinr_canyougetavisualonem", "s_iinr_ineedyoutocheckitout"];
    level.battlechatter.eventstrings["KON_L"]["inv_patrol_noresp"] = ["s_iinr_thresherisntrespondi", "s_iinr_thresherdidntcheckin", "s_iinr_thresherisntoncomms", "s_iinr_icantreachthresher"];
    level.battlechatter.eventstrings["KON_L"]["report_search"] = ["s_aias_sitrep", "s_aias_whatsyourstatus", "s_aias_findanything", "s_aias_gotanything", "s_aias_status"];
    level.battlechatter.eventstrings["KON_L"]["report_alert"] = ["s_airp_allshadowsgetswitche", "s_airp_allshadowsperimeterm", "s_airp_allshadowspossibleen", "s_airp_allshadowssetsecurit"];
    level.battlechatter.eventstrings["KON_L"]["warn_reply"] = ["s_wrpl_youreclearedhotifthe", "s_wrpl_youseeemagainputemin", "s_wrpl_theycomebackyourecle"];
    level.battlechatter.eventstrings["KON_L"]["report_target_location"] = ["s_chrp_doesanyonehaveavisua", "s_chrp_talktomeguyswhatsthe", "s_chrp_cmonshadowsineedaloc", "s_chrp_allshadowsdoyouhavea"];
    level.battlechatter.eventstrings["KON_L"]["report_idle"] = ["s_irpr_status", "s_irpr_sitrep", "s_irpr_yougood", "s_irpr_gotanything", "s_irpr_report"];
    level.battlechatter.eventstrings["KON_L"]["ally_missing_multi"] = ["s_alms_multipleshadowsarent", "s_alms_moreshadowsarentresp", "s_alms_wevelostcontactwithm", "s_alms_anotherteamlostcomms"];
    level.battlechatter.eventstrings["KON_L"]["order_search_start"] = ["s_aior_searchthearea", "s_aior_pullsecuritysweepthe", "s_aior_securetheperimeter"];
    level.battlechatter.eventstrings["KON_L"]["order_report_back"] = ["s_aior_soundoffifyoufindsom", "s_aior_imstandingbyifyoufin", "s_aior_usecommsifyoufindsom"];
    level.battlechatter.eventstrings["KON_L"]["hunt_patrol_noresp"] = ["s_hpnr_thresherisntrespondi", "s_hpnr_thresherdidntcheckin", "s_hpnr_thresherisntanswerin", "s_hpnr_icantreachthresher"];
    level.battlechatter.eventstrings["KON_L"]["leader_ask_corpse_name"] = ["s_alrc_whoisit", "s_alrc_whodidwelose", "s_alrc_whoisitthatwelost"];
    level.battlechatter.eventstrings["KON_L"]["leader_revenge"] = ["c_aded_iknowyoureoutthereju", "c_aded_youthinkthatyouvewon", "c_aded_takethissmallwinwhil"];
    level.battlechatter.eventstrings["KON_L"]["leader_revenge_backupinc"] = ["c_adbu_enjoythesilencewhile", "c_adbu_youthoughtyoucouldki", "c_adbu_ifiwereyouidrunbefor"];
    level.battlechatter.eventstrings["KON_L"]["report_alert_combat"] = ["c_actr_allsoldiersenemyisin", "c_actr_anenemyhasbreachedth", "c_actr_allsoldiersperimeter"];
    level.battlechatter.eventstrings["KON_L"]["order_attack"] = ["c_ator_engage", "c_ator_gohot", "c_ator_youreclearedhot"];
    if (!utility::ismp()) {
