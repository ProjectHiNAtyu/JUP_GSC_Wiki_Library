// scripts\mp\bots\bots.gsc  (266C7A1B794E081D.gscc) (size: 64899 Bytes / 0xfd83 / GSC)
// magic .... 0xa0d4353478a vm: Call of Duty: Modern Warfare III (PC)
// crc: 0xdef06a0d (3740297741)
// size ...... 64899 (0xfd83)
// includes .. 37  (offset: 0xabf)
// strings ... 171 (offset: 0xedc3)
// dev strs .. 72  (offset: 0xf8a7)
// exports ... 149 (offset: 0xa0c7)
// cseg ...... 0xbe7 + 0x94e0 (0xa0c7)
// imports ... 422 (offset: 0xb80f)
// animtree1 . 0   (offset: 0xfd83)
// animtree2 . 0   (offset: 0xfd83)
...
// Size: 0x13c
function bot_damage_callback(eattacker, idamage, smeansofdeath, sweapon, einflictor, shitloc, vpoint) {
    ...
    while (true) {
        ...
        foreach (crate in var_9e543bde5d2a2496) {
                ...
                if (var_2df5bad592ba26ac.size > 0) {
                    ...
                    if (isdefined(var_f8b50191af8c5b3c)) {
                        ...
                        foreach (node in crate.nearest_nodes) {
                            var_10256e13fb6f7945 |= nodesvisible(var_f8b50191af8c5b3c, node, 1);
                        }
                    }
                }
            }
            if (!var_10256e13fb6f7945) {
                ...
                foreach (node in crate.nearest_nodes) {
                    var_539c5e6b72fb319 |= nodesvisible(var_b6f1deb8e946e67c, node, 1);
                }
...
/#
    ...
    // Namespace bots / scripts\mp\bots\bots
    // Params 7
    // Checksum 0x0, Offset: 0x9a8c
    // Size: 0x148, Type: dev
    function function_b09b8c00320b91e(eattacker, idamage, smeansofdeath, objweapon, einflictor, shitloc, vpoint) {
        host = scripts\mp\dev::devfindhost();
        if (!isdefined(host) || eattacker != host) {
            return;
        }
        self.var_2fc19f3a438f03a5++;
        eye = eattacker geteye();
        range = int(distance(eye, vpoint));
        self.menu[self.var_eb76b43f1e36badd] setvalue(self.health);
        self.menu[self.var_c1b4ab740589a6da] setvalue(idamage);
        self.menu[self.var_211d9fb4cbdadf3d] setvalue(self.var_2fc19f3a438f03a5);
        self.menu[self.var_c47100e5d06a16b0] setvalue(range);
        self.menu[self.var_4f1789c922ba408a] setdevtext(shitloc);
        self.menu[self.var_b886fd1c41ae3a0b] setdevtext(objweapon.basename);
    }
    ...
    // Namespace bots / scripts\mp\bots\bots
    // Params 1
    // Checksum 0x0, Offset: 0x9cb8
    // Size: 0xd8, Type: dev
    function function_5723f098aabd9836(host) {
        host endon("<dev string:x2ad>");
        host endon("<dev string:x2a2>");
        placementmodel = spawn("<dev string:x474>", (0, 0, 0));
        placementmodel setmodel("<dev string:x481>");
        for (;;) {
            waitframe();
            spawnorigin = function_b771795f421ef637(host);
            if (!isdefined(spawnorigin)) {
    ...
    // Namespace bots / scripts\mp\bots\bots
    // Params 1
    // Checksum 0x0, Offset: 0x9d98
    // Size: 0x20e, Type: dev
    function function_e53f5c23f0a7a78f(host) {
        x = -40;
        y = 200;
        self.hud = host function_829a24c34a7fda26("<dev string:x491>", x, y, 1, 0.6, 10);
        menu = [];
        menux = x + 5;
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x49c>", menux, y + 10, 1);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4a4>", menux, y + 20, 1);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4ac>", menux, y + 30, 1);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4bc>", menux, y + 40, 1);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4c3>", menux, y + 50, 1);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4d1>", menux, y + 60, 1);
        self.var_eb76b43f1e36badd = menu.size;
        self.var_c1b4ab740589a6da = menu.size + 1;
        self.var_211d9fb4cbdadf3d = menu.size + 2;
        self.var_c47100e5d06a16b0 = menu.size + 3;
        self.var_4f1789c922ba408a = menu.size + 4;
        self.var_b886fd1c41ae3a0b = menu.size + 5;
        menux = x + 70;
        menu[self.var_eb76b43f1e36badd] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10, 1);
        menu[self.var_c1b4ab740589a6da] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 20, 1);
        menu[self.var_211d9fb4cbdadf3d] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 30, 1);
        menu[self.var_c47100e5d06a16b0] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 40, 1);
        menu[self.var_4f1789c922ba408a] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 50, 1);
        menu[self.var_b886fd1c41ae3a0b] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 60, 1);
        self.menu = menu;
    }
    ...
    function function_829a24c34a7fda26(text, x, y, scale, alpha, sort) {
        ...
        hudelem.elemtype = "<dev string:x4d9>";
        hudelem.alpha = alpha;
        ...
        hudelem.y = y;
        hudelem.width = 0;
        hudelem.height = int(level.fontheight * scale);
        if (isdefined(text)) {
...
// Size: 0x5
function function_b780e59d6f5457ea() {
    /#
    #/
}
