// script_46307eda1984556f  (46307EDA1984556F.gscc) (size: 25323 Bytes / 0x62eb / GSC)
// magic .... 0xa0d4353478a vm: Call of Duty: Modern Warfare III (PC)
// crc: 0xb43fd1c4 (3024081348)
// size ...... 25323 (0x62eb)
// includes .. 19  (offset: 0x74c)
// strings ... 100 (offset: 0x5c5f)
// dev strs .. 13  (offset: 0x623f)
// exports ... 65  (offset: 0x3667)
// cseg ...... 0x7e4 + 0x2e83 (0x3667)
// imports ... 235 (offset: 0x408f)
// animtree1 . 0   (offset: 0x62eb)
// animtree2 . 0   (offset: 0x62eb)
...
function function_7290f6d8c7a2a47b(var_39a0d8ee3716ac3a) {
    ...
    assertex(isdefined(level.vehicle), "cougar_init() called before vehicle_init().");
    callbacks = [];
...
function private function_96c9a47c791faad7(vehicle, seatid, var_fc7c7a874b43a31a, player, data) {
    if (seatid == "driver") {
        ...
        player thread scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_giveturret(player, vehicle, turretweapon);
...
function private function_8ae7c9135db427e7() {
    assertex(isdefined(level.vehicle), "cougar_init() called before vehicle_init().");
    assertex(isdefined(level.vehicle.cougar), "cougar_init() called before cougar_init().");
...
/#
    ...
    function private function_31ee9844edbb6426(spawndata) {
        ...
        if (!isdefined(spawndata.spawnmethod)) {
            spawndata.spawnmethod = "<dev string:x2d>";
            return 1;
        }
        if (spawndata.spawnmethod != "<dev string:x3f>" && spawndata.spawnmethod != "<dev string:x53>" && spawndata.spawnmethod != "<dev string:x67>" && spawndata.spawnmethod != "<dev string:x2d>" && spawndata.spawnmethod != "<dev string:x82>") {
            thread error("<dev string:x9b>");
            return 0;
        }
        if (spawndata.spawnmethod == "<dev string:x3f>") {
            if (!isdefined(spawndata.owner)) {
                thread error("<dev string:x142>");
                return 0;
            }
        } else if (spawndata.spawnmethod == "<dev string:x53>") {
            if (!isdefined(spawndata.origin) || !isdefined(spawndata.angles)) {
                thread error("<dev string:x194>");
                return 0;
            }
        } else if (spawndata.spawnmethod == "<dev string:x2d>") {
            if (!isdefined(spawndata.origin)) {
                thread error("<dev string:x1fc>");
                return 0;
            }
        }
        if (issubstr(spawndata.spawnmethod, "<dev string:x24d>")) {
            if (!isdefined(spawndata.origin) || !isdefined(spawndata.angles)) {
                thread error("<dev string:x255>");
                return 0;
            }
        }
    }
