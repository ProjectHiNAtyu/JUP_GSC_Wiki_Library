// script_31629c16ddf56ebc  (31629C16DDF56EBC.gscc) (size: 29323 Bytes / 0x728b / GSC)
// magic .... 0xa0d4353478a vm: Call of Duty: Modern Warfare III (PC)
// crc: 0x52c219e8 (1388452328)
// size ...... 29323 (0x728b)
// includes .. 19  (offset: 0x8f5)
// strings ... 121 (offset: 0x6a37)
// dev strs .. 15  (offset: 0x71cb)
// exports ... 76  (offset: 0x3f2b)
// cseg ...... 0x98d + 0x359e (0x3f2b)
// imports ... 262 (offset: 0x4b0b)
// animtree1 . 0   (offset: 0x728b)
// animtree2 . 0   (offset: 0x728b)
...
function private light_tank_init() {
    ...
    assertex(isdefined(level.vehicle), "light_tank_init() called before vehicle_init().");
    assertex(!isdefined(level.vehicle.lighttank), "light_tank_init() should only be called once.");
    callbacks = [];
    ...
    light_tank_initvo();
    /#
        issharedfuncdefined("<dev string:x1c>", "<dev string:x21>", 1);
    #/
}
...
function private light_tank_monitordrivermissilefire(player) {
    ...
    while (true) {
        ...
        if (gettime() - self.lastmissilefired >= 1330) {
            ...
            /#
                player iprintlnbold("<dev string:x2d>");
            #/
        }
    }
}
...
function private light_tank_getleveldata() {
    assertex(isdefined(level.vehicle), "light_tank_init() called before vehicle_init().");
    assertex(isdefined(level.vehicle.lighttank), "light_tank_init() called before light_tank_init().");
...
/#
    ...
    function private function_b6f3b5fca0f8c2b2(spawndata) {
        ...
        if (!isdefined(spawndata.spawnmethod)) {
            thread error("<dev string:x3d>");
            return 0;
        }
        if (spawndata.spawnmethod != "<dev string:x64>" && spawndata.spawnmethod != "<dev string:x78>" && spawndata.spawnmethod != "<dev string:x8c>" && spawndata.spawnmethod != "<dev string:xa7>" && spawndata.spawnmethod != "<dev string:xb9>") {
            thread error("<dev string:xd2>");
            return 0;
        }
        if (spawndata.spawnmethod == "<dev string:x64>") {
            if (!isdefined(spawndata.owner)) {
                thread error("<dev string:x179>");
                return 0;
            }
        } else if (spawndata.spawnmethod == "<dev string:x78>") {
            if (!isdefined(spawndata.origin) || !isdefined(spawndata.angles)) {
                thread error("<dev string:x1cb>");
                return 0;
            }
        } else if (spawndata.spawnmethod == "<dev string:xa7>") {
            if (!isdefined(spawndata.origin)) {
                thread error("<dev string:x233>");
                return 0;
            }
        }
        if (issubstr(spawndata.spawnmethod, "<dev string:x284>")) {
            if (!isdefined(spawndata.origin) || !isdefined(spawndata.angles)) {
                thread error("<dev string:x28c>");
                return 0;
            }
        }
    }
